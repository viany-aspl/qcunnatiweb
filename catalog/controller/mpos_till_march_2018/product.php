...
'filter_role'=>$mcrypt->decrypt($this->request->post['rid'])
	...
	foreach ($categories as $category_info) 
	{
		...
		$json['navigation'][] = array(
					...
					'children'   => $this->model_catalog_storemenu->getChildCategories($category_info['category_id'],$mcrypt->decrypt($this->request->post['store_id']),$mcrypt->decrypt($this->request->post['rid']))
...
public function products() 
{
	...
	if (!isset($this->session->data['api_id'])) 
	{
			$json['error']['warning'] = $this->language->get('error_permission');
              ...
              {
		$log =new Log("prd-".date('Y-m-d').".log");
