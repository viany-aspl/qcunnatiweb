...
$log=new Log("getFmType-".date('Y-m-d').".log");
		...
		$json = array('success' => true, 'metadata' => array());
			...
			$json['metadata'][] = array(
                        ...
                        'name'       =>$mcrypt->encrypt($ids['FM_NAME']),
                                     );
				}
...
function testforone()
 {
	  echo $this-> decryptRJ256('9PLN4ZDPBWVkzN7wD0scYeQOsCu+ziPJYSJSGVtJXg4='); 
	exit;
	$this->adminmodel('pos/pos');
	
	
	$order_details=$this->model_pos_pos->getOrder(121278);
		
		
		echo $this->request->post['store_id']=$order_details['store_id'];
		$this->request->post['order_id']=121278;
		$order_details=$this->model_pos_pos->updateinventory($this->request->post);
 }
}
?>