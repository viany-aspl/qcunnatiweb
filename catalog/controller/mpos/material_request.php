...
error_reporting(0);
class ControllerMposMaterialRequest extends Controller{
    ...
    public function adminmodel($model) {
      ...
      if (file_exists($file)) {
        ...
        /*getting the list of the orders*/
                 $mcrypt=new MCrypt();
		$this->adminmodel('materialrequest/purchase_order');
		...
		$log=new Log("inv-material-request-order-list".date('Y-m-d').".log");
		$start = ($page-1)*20;
		$limit = 20;
		...
		/*getting the list of the orders*/
                $da_res_array = array('start'=>$start,'limit'=>$limit,'uid'=>$uid,'status'=>$ostatus);
                
		$log->write($da_res_array);
		$da_data=$this->model_materialrequest_purchase_order->getListRecStore($da_res_array);
		$log->write($da_data);
		...
		foreach($da_data as $d_data)    
		{
                   ...
                   // $product_id="517";
                    ...
                    $product_id =$d_data['product_id'];
                    $stock=$this->model_materialrequest_purchase_order->getstock($product_id,$store_id);
                    $get_store_name=$this->model_materialrequest_purchase_order->get_store_data($store_id,'name');
                    $log->write($get_store_name);
                    $stores_name=$get_store_name[0]['name'];
                    $log->write($stores_name);  
//                    $log->write($stock);                
...
/*----------------------------view_order_details function starts here------------*/
    public function orderlistAll()
    {
        ...
        $log=new Log("material_request-RH-list-storewise-".date('Y-m-d').".log");
        $start = ($page-1)*20;
        $limit = 20;
        $log->write('orderlistAll called for seacrh material request');
        $log->write($this->request->post);
       
        $uid=$mcrypt->decrypt($this->request->post['username']); 
        //$order_id=$mcrypt->decrypt($this->request->post['order_id']); 
        $store_id=$mcrypt->decrypt($this->request->post['store_id']); 
        $log->write('store_id-'.$store_id.' && username-'.$uid);
          $ostatus=0;
               if($ptype=='SRA')
               {$ostatus=1;} 
               
        $da_res_array = array('start'=>$start,'limit'=>$limit,'store_id'=>$store_id,'status'=>$ostatus);
        $da_data=$this->model_materialrequest_purchase_order->getListRecStore($da_res_array);
       // $da_data=$this->model_materialrequest_purchase_order->getListStore($uid,$start,$limit,$store_id);
        $log->write($da_data);
       ...
       foreach($da_data as $d_data)
		{
                    ...
                    $get_user_credit=$this->model_user_user->get_user_credit($d_data['user_id']);
//                    $log->write('vinay65');
//                    $log->write($d_data['user_id']);
                    $log->write($get_user_credit);
                    if ($get_user_credit['status']== 'success'){
                         $user_max_qunatity=$get_user_credit['response'][0]['user_max_qunatity'];
                         $max_credit_amount=$get_user_credit['response'][0]['max_credit_amount'];
                    } else {
                       $user_max_qunatity='0';
                       $max_credit_amount= '0';
                     }
...
//                    $log->write($stock);                
//                    $log->write(($stock[0]['quantity'])); 
                    $stock_qty=(is_array($stock) && !empty($stock))?$stock[0]['quantity']:'0';
                    $stock_price=(is_array($stock) && !empty($stock))?$stock[0]['quantity']*$stock[0]['store_price']:'0';
...
//                    $log->write($stock[0]['store_price']);
		 ...
		 $data['order_list'][]=array
                                        (
                                            ...
                                            'quantity' => $mcrypt->encrypt($d_data['quantity']),
                                            'name' => $mcrypt->encrypt($d_data['name']),
                                            'stock' =>$mcrypt->encrypt($stock_qty),
                                            'stock_price' =>$mcrypt->encrypt($stock_price),
                                            's_name' =>$mcrypt->encrypt( $stores_name),
                                            'user_max_qunatity' => $mcrypt->encrypt( $user_max_qunatity),
                                            'max_credit_amount' => $mcrypt->encrypt($max_credit_amount),
                                            'username' => $mcrypt->encrypt($d_data['username'])
...
/*----------------------------view_order_details function starts here------------*/
	...
	public function receive_order()
	{
		...
		{
			$database_quantity= $this->model_materialrequest_purchase_order->check_quantity($order_id); 
		...
		$log->write($user_id."-".$order_id."-".$driver_otp);
                            ...
                            $database_otp= $this->model_materialrequest_purchase_order->check_driver_otp($order_id); 
		...
		else
		{
			...
			$this->adminmodel('materialrequest/purchase_order');
			$checkAccepted=$this->model_materialrequest_purchase_order->getList_order($order_id);
	...
	/*--------------------Insert Purchase Order starts heres-------------------------------------------------*/
            ...
            {
                $log=new Log("ZI material request ".date('Y-m-d')." .log");
                     $mcrypt=new MCrypt();
                   ...
                   $price=array();
                    ...
                    $log->write('Load admin modal end');                                                       
...
//                                    }        
                                    ...
                                    //check for old purchase order is pending or not
                                      $pending_orders = $this->model_materialrequest_purchase_order->getList_pending_order($store_id);
...
//                                        $json['error'] = $mcrypt->encrypt('Error: Please contact your regional head and cane head for credit limit adjustment.');
//                                        $this->response->setOutput(json_encode($json));	
//                                        return;                                        
//                                        }
                                     $log->write("check1");
	...
	//otp for order
        public function message()
        {
            ...
            $data['user_information'] =$this->model_materialrequest_purchase_order->get_driver_otp($order_id);
            ...
            if($otp!=$database_otp)
		{
             ...
             //$sms=new sms($this->registry);
             //$sms->sendsms($this->request->post['customer_mob'],"2",$data);
            $json['success'] = ('Success: OTP verified for Order '.$order_id.' successfully');
        ...
        {                           
                $mcrypt=new MCrypt();        
                $log=new Log("mpos-approve-order-by-rh-". date('Y-m-d').".log");
                $log->write("in");	
                $log->write( $this->request->post);
                ...
                $setcollum= "order_status_id ='".$order_status."', receive_bit ='1', receive_date = NOW() ";
		$msg =$this->model_materialrequest_purchase_order->change_order_status($order_id,$setcollum);	
		$log->write($msg);		
                $json['success'] = ('Success: '.$msg);
                $json['message'] = ('Success: '.$msg);
                $this->response->setOutput(json_encode($json));
