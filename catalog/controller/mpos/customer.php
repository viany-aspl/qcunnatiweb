...
class ControllermposCustomer extends Controller {
	...
	public function sendotp()
	{
		...
		if(empty($this->request->post['fm_mobile']) || ($this->request->post['fm_mobile']=='-1'))
		{
			$json['status']="0";
            $json['msg'] = ('Mobile can not be empty');
		...
		if(!empty($query_return))
        {
                ...
                $data=array();
                $data['mpin']=$this->request->post['otp'];
                $log->write('before sending to sms');
                $log->write($data);
                $sms->sendsms($mobile,"25",$data);
	...
	{
		$log=new Log("generatempin-".date('Y-m-d').".log");
        $mcrypt=new MCrypt();
	    ...
	    $db_data=$this->model_user_user->generatempin(array('user_id'=>$user_id,'user_group_id'=>36));
		//$json=array();
		//$log->write($json);
		if($db_data)
		{
		$this->response->setOutput(json_encode(array('status'=>1,'msg'=>'MPIN sent to your registered mobile number')));
		}
		else
		{
			$this->response->setOutput(json_encode(array('status'=>0,'msg'=>'Oops ! Some error occur.Please try again.')));
		}
	}
	...
	{
			$log=new Log("getUnitsbyCompany-".date('Y-m-d').".log");
            $mcrypt=new MCrypt();
	        $store_id=$mcrypt->decrypt($this->request->post['store_id']);
            $company_id=$mcrypt->decrypt($this->request->post['companyid']);
			$log->write($this->requiest->post);
	        ...
	        $db_data=$this->model_setting_store->getUnitsbyCompany($company_id);
			$log->write($db_data);
			$json=array(); 
			foreach($db_data as $dbd)
			{
...
public function setgeo()
{
	...
	public function sendtorunner()
    {
	    ...
	    $this->adminmodel('pos/pos');			       
			...
			$query_return=$this->model_pos_pos->insert_runner_otp($data,$data['storeid'],$pin);
			$log->write($query_return);
			if($query_return>0)
			{
				$log->write('return data: 1');
				if(isset($this->request->post['subuser_id']))
				{
				$data['order_id']=$query_return;
		...
		$log->write($this->request->post);
			...
			if(!empty($data['amount']) && !empty($data['store_id'])){ 
                                           ...
                                           if(($current_cash<$data['amount'])  && ($data['bank_id']!="4"))
                                           {
                                               ...
                                               $json['error'] = 'Error: Amount can not be greater then Cash in Hand.';
                                           }
             			else
             			{
								$jsons = $this->model_account_customer->addbankTrans($data);
								$log->write($jsons);
								if($this->request->post['utye']==36)
								{
									$inserted= $this->model_account_customer->add_to_store_cash($data['ce_id'],$data['amount'],$jsons,$data['store_id']); 
								...
								////send mail to admin////
								$this->send_accept_mail_to_admin($data['ce_id'],$data['amount'],$data['store_id'],$data['user_id'],$current_cash,$this->request->post['utye']);
								/////////////
				...
				$json['error'] = 'Error: Amount can not be zero.';
				}
				if(empty($data['store_id'])){
				$json['error'] = 'Error: You are not authorized.';
				}
