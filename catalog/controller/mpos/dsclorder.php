...
class Controllermposdsclorder extends Controller {
	...
	public function sendorder() {
		...
		$log=new Log("sms-tagging-".date('Y-m-d').".log");			
		 $mcrypt=new MCrypt();
...
foreach ($keys as $key) {
		...
		$qcheck=false;
				...
				$activity_data = array(
					'customer_id' => $mcrypt->decrypt($this->request->post['user_id']),
					'data'        => json_encode($this->request->post),
				);

				$this->model_account_activity->addActivity('dOrderQuantity', $activity_data);
...
//check for product quantity
				...
				foreach($prds as $prd)
					{		
						...
						{
							$datav.="<h3> Error in order process</h3><br/>";
							$datav.="<table>";
							$datav.="<tr>";
							$datav.="<td> <b>Product Name </b> </td> <td> :   </td><td>".$product_info['name']."</td></tr>";
							$datav.="<tr><td> <b> Store Quantity </b> </td> <td> : </td><td>".$product_info['squantity']."</td></tr>";
							$datav.="<tr><td> <b> Your Quantity </b> </td> <td> : </td><td>".$prd['product_quantity']."</td></tr>";
							$datav.="</table>";

							$qcheck=true;
							$json['error']=$datav;//"".$prd['product_quantity']." quantity for ".$product_info['name']." not match with system";
					                $json['success'] = "-1";
							$this->response->setOutput(json_encode($json));	
							return;
				         	}         
...
//end quantity check end
	...
	//add data to cart
		...
		$data['shipping_address_2'] = $this->request->post['uid'];
		$data['shipping_city'] = '';
...
{
 $log=new Log("dscl-CardRequest-".date('Y-m-d').".log"); 
 $log->write($this->request->post);
 $StatusId='1';
 ...
 $keys = array(
	...
	$log->write($this->request->post);
	$this->model_farmerrequest_farmerrequest->addCarddetail($this->request->post,$StatusId,$StatusName);
	//$json['success'] =1;
	$log->write('1');
...
public function rejectstatusupdate()
{
 $log=new Log("dscl-rejectstatusupdate-".date('Y-m-d').".log"); 
 $log->write($this->request->post); 
 $this->adminmodel('farmerrequest/farmerrequest');
 ...
 $keys = array(
	...
	$log->write($this->request->post);
	$this->model_farmerrequest_farmerrequest->updateRejectStatusNet($this->request->post);
	//$json['success'] =1;
	$log->write('1');
	$this->response->setOutput('1');
...
public function approvestatusupdate()
{
 $log=new Log("dscl-approvestatusupdate-".date('Y-m-d').".log"); 
 $log->write($this->request->post); 
 $this->adminmodel('farmerrequest/farmerrequest');
 ...
 $keys = array(
	...
	foreach ($keys as $key) {
		...
		$this->request->post['card_data']=$mcrypt->decrypt($this->request->post['card_data']);
	$log->write($this->request->post);
	$dataStatus=$this->model_farmerrequest_farmerrequest->updateApprovedStatusNet($this->request->post);
	$log->write($dataStatus);
		

	//$json['success'] =1;
	$log->write($dataStatus);
	$this->response->setOutput($dataStatus); 
