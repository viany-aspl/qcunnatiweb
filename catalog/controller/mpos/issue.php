<?php
class ControllermposIssue extends Controller {
	...
	{
		$log=new Log("test".date('Y-m-d').".log");
		$this->adminmodel('pos/dscl');
		$log->write("dscl_count in");
				$matcode=100000+357;
	...
	if(!empty($this->request->server['HTTP_UPN'])) 
	{
		...
		if(!empty($unitdata['company_name']))
		{
			...
			$dataAuthentication = array('CARD_STATUS'=>9,'GROWER_ID'=>$UPN);//$this->{'model_pos_' . $company}->GetAuthentication('GetAuthentication',$this->request->post,0);
		...
		$dscl_count = $this->model_pos_dscl->GetCardDataSql('GetCardDataSql',$filter_data_sql,0);
			*/
			if(($company=="dscl") && ($this->request->post['payment_method']!="Cash"))
			{
				...
				$log->write($filter_data_sql);
				$log->write("before GetCardDataSql ");
				$dscl_count = 0;//$this->model_pos_dscl->GetCardDataSql('GetCardDataSql',$filter_data_sql,0);
				$log->write("dscl_count= ");
				$log->write($dscl_count);
				if(0)//empty($dscl_count[0]['CNT']))
				{
					$log->write("in check if");
					//not save data
					$json['error']=" No product (".$product_info['name'].") found at cane server";
...
if($payment_method=='Cash Subsidy')
		...
		if($payment_method=='Cheque')
		{
			...
			$data['cheaccno']=$mcrypt->decrypt($this->request->post['cheaccno']);
                ...
                $order_id = $this->model_pos_pos->addOrderIssue($data);
		...
		try{
			...
			//$log->write($datares);
				$log->write('before send to confirm_ coupon');
				$log->write($this->request->post['prdsub']);
				if((!empty($this->request->post['prdsub'])))//(!isset($this->request->post['prdsub'])) && 
				{
					$log->write('in ifconfirm_ coupon');
