...
error_reporting(0);
class ControllerMposPrWarehouseInventory extends Controller{
...
public function orderlist()
	{
					...
					/*getting the list of the orders*/
						 $mcrypt=new MCrypt();
		$this->adminmodel('prwarehouseinventory/purchase_order');
		...
		$uid=$mcrypt->decrypt($this->request->post['username']);
               ...
               if($ptype=='SRA')
               {$ostatus=1;}
		//$log->write($this->model_prwarehouseinventory_purchase_order->getListRecStore($uid,$start,$limit));
		//$data['order_list'] =$mcrypt->encrypt(serialize( $this->model_inventory_purchase_order->getListRecStore($uid,$start,$limit)));
		/*getting the list of the orders*/
           
		$da_data=$this->model_prwarehouseinventory_purchase_order->getListRecStore($uid,$start,$limit,$ostatus);
		...
		foreach($da_data as $d_data)
		{
                    ...
                    $stock=$this->model_prwarehouseinventory_purchase_order->getstock($product_id,$store_id);
                    $get_store_name=$this->model_prwarehouseinventory_purchase_order->get_store_name($store_id);
...
public function orderlistAll()
    {
        ...
        $da_data=$this->model_prwarehouseinventory_purchase_order->getListStore($uid,$start,$limit,$order_id);
        $log->write($da_data);
...
/*----------------------------view_order_details function starts here------------*/
	...
	public function order_details()
	{
		...
		$log->write($order_id);							
		$this->adminmodel('prwarehouseinventory/purchase_order');
		$data['order_information'] =$this->model_prwarehouseinventory_purchase_order->view_order_details($order_id);		
	...
	public function receive_order()
	{
		...
		{
			$database_quantity= $this->model_prwarehouseinventory_purchase_order->check_quantity($order_id); 
		...
		$log->write($user_id."-".$order_id."-".$driver_otp);
                            ...
                            $database_otp= $this->model_prwarehouseinventory_purchase_order->check_driver_otp($order_id); 
		...
		else
		{
			...
			$this->adminmodel('prwarehouseinventory/purchase_order');
			$checkAccepted=$this->model_prwarehouseinventory_purchase_order->getList_order($order_id);
			$log->write($checkAccepted);////this log is added by vipin at 22-may-2018-2.43 pm
			if(!$checkAccepted){
			$inserted = $this->model_prwarehouseinventory_purchase_order->insert_receive_order($received_order_info,$order_id);
			if($inserted)
			{
	...
	/*--------------------Insert Purchase Order starts heres-------------------------------------------------*/
            ...
            public function warehouse_request_order()
            {
                    $log=new Log("warehouse-pr-request".date('Y-m-d').".log");
                    $mcrypt=new MCrypt();
...
//                    $data['option_values'] = $_POST['option_values'];
                    ...
                    $data['id_prefix']=$_POST['id_prefix'];
                    $data['sid']=$_POST['sid'];
                   // $data['imagename'] = $mcrypt->decrypt($_POST['image_name']);
                   // var_dump($_POST);
                    
                    $log->write("check price");
                    $log->write($data['price']);                    
                    $data['price']=$mcrypt->decrypt($data['price']);
                    ...
                    /*to let the user add products without option values End*/
                 ...
                 //  $log->write("befor if check ".count($data['quantity']). ' & '.count(array_filter($data['quantity'])). ' & '.in_array("--products--",$data['products']));
                    if($data['products'] != '' && $data['stores'] != '' && $data['quantity'] > 0 && $data['price']>0)
                    {
                            $log->write("in if");
                            $iq = 0;
                            ...
                            $product_names = explode('_',$product);
                               ...
                               $log->write("prd id-".$mcrypt->decrypt($productval[0]));
                                ...
                                //check for old purchase order is pending or not
                                  $pending_orders = $this->model_prwarehouseinventory_purchase_order->getList_pending_order($store_id,$mcrypt->decrypt($productval[0]));
...
');
                   ...
                   $log->write("Before po number");
                    $po_number=$mcrypt->decrypt($data['id_prefix']).$mcrypt->decrypt($data['sid']); 
                    $data['po_number'] = $po_number;
                    $data['potentialdate'] = $po_number;
                   $log->write("After po number :".$po_number);  
                    
                            $this->adminmodel('prwarehouseinventory/purchase_order');
                              $log->write("after");
                            $order_id = $this->model_prwarehouseinventory_purchase_order->insert_purchase_order($data);
                    ...
                    else
                    {
                            ...
                            $json['error'] = $mcrypt->encrypt('Fail: Order can not processed: ');
                            $this->response->setOutput(json_encode($json));
                           // exit;
	...
	///////////////////////////////////////////////////////////////////////////////////
          ...
          // change order status  
        public function po_reject_order()
        {                           
                $mcrypt=new MCrypt();        
                $log=new Log("po_reject_order.log");
                $log->write("in");	
                $log->write( $this->request->post);
                ...
                $this->adminmodel('prwarehouseinventory/purchase_order');
		$data['order_information'] =$this->model_prwarehouseinventory_purchase_order->po_reject_order($order_id,$order_status);	
		$log->write($data['order_information']);		
	...
	//otp for order
        public function message()
        {
            ...
            $data['user_information'] =$this->model_prwarehouseinventory_purchase_order->get_driver_otp($order_id);
