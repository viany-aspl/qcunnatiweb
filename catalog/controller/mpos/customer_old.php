...
*/
$news = simplexml_load_file('https://news.google.com/news?cf=all&hl=hi&pz=1&ned=hi_in&output=rss&q=agriculture');
...
public function getUnitsbyStore(){
			...
			{
				$json[]=array('unit_id'=>$dbd['unit_id'],'unit_name'=>$dbd['unit_name']);
			}
			$log->write($json);
            $this->response->setOutput(json_encode($json));
}

public function getGrowerbyUnits(){	
		$json = array();
		$log=new Log("getGrowerbyUnits.log");
		$log->write($this->request->post);
		 $mcrypt=new MCrypt();
				//log to system table
				$this-> adminmodel('pos/pos');
				...
				$up=$this->model_pos_pos->getgrower_on_unit($json);
				
				
				 $this->response->setOutput($up);
}
