...
class Controllermpossubuser extends Controller 
{
	...
	public function getallstoreincharge() 
	{
		...
		$log->write($cash);
		$data['cash']=$mcrypt->encrypt(str_replace(",","",$this->currency->format($cash))); 
		$data['results'] = array();
	...
	public function getStoresubuserSummarydtl() 
	{
		...
		$log->write($user_id);
		if(!empty($mcrypt->decrypt($this->request->post['subuser_id'])))
		{
			$user_id =$mcrypt->decrypt($this->request->post['subuser_id']);
		}
		$log->write($user_id);
		$log->write($this->request->post);
	...
	public function subuser_farmercash_otp()
    {
	   ...
	   // $this->adminmodel('pos/pos');
		$this->load->model('account/subuser');		
			...
			$mobile=$ids['username'];//'9911427348';// '7050101560';//
			$log->write($mobile);
			if(strlen($mobile)==9)
			{
				$mobile=$ids['email']; 
	...
	public function getSubUser()
	{
		...
		public function material_summary()
		{
			...
			if ($this->request->post['start_date']!='00-00-00000') 
			{
				$data['start_date']=$start_date = ($this->request->post['start_date']);
			} 
			else 
			{
				$data['start_date']=$start_date = (date('Y')-1).'-10-01';
			}
			if ($this->request->post['end_date']!='00-00-00000') 
			{
				$data['end_date']=$end_date = ($this->request->post['end_date']);
			} 
			else 
			{
				$data['end_date']=$end_date = date('Y-m-d');
			}
		...
		public function material_detail()
		{
			...
			{
				$data['start_date']=$start_date = $mcrypt->decrypt($this->request->post['start_date']);
			} 
			else 
			{
				$data['start_date']=$start_date = (date('Y')-1).'-10-01';
			...
			{
				$data['end_date']=$end_date = $mcrypt->decrypt($this->request->post['end_date']);
			} 
			else 
			{
				$data['end_date']=$end_date = date('Y-m-d');
	...
	public function request_order_trans()
	{
		...
		foreach($data['stores'] as $store)
			...
			foreach($data['products'] as $product)
			{
				...
				$products[]=$dataprd;
				$smsproduct.= $dataprd['product_name'].",".$dataprd['quantity']."-";				
			}
			...
			if(!empty($query_return))
			{
				...
				$data['products']=substr($smsproduct,0,-1);
				...
				$json['success'] = $mcrypt->encrypt('Success: new order placed with ID: '.$query_return);						
				   ...
				   $json['error'] = ('Error in submission');
					...
					$json['success']="";
				   $json['error'] = ('One item transfered at one time');
			}			
			$this->response->setOutput(json_encode($json));
	...
	public function getBilledMaterial()
	{
		...
		if ($this->request->post['start_date']!='00-00-00000') 
			{
				$filter_data['start_date']=$start_date = ($this->request->post['start_date']);
			} 
			else 
			{
				$filter_data['start_date']=$start_date = (date('Y')-1).'-10-01';
			}
			if ($this->request->post['end_date']!='00-00-00000') 
			{
				$filter_data['end_date']=$end_date = ($this->request->post['end_date']);
			} 
			else 
			{
				$filter_data['end_date']=$end_date = date('Y-m-d');
			}
		...
		{		
			$this->load->model('account/subuser');
			$results=$this->model_account_subuser->getBilledMaterial($contrator_id,$filter_data); 
			//$log->write($results);  //print_r($results);
	...
	{  
		...
		{	   
			$this->load->model('account/subuser');
			$results=$this->model_account_subuser->getBilledMaterialproductdtl($productid,$contrator_id,$filter_data); 
			$log->write($results);  //print_r($results);
...
/**********Billed Material Report*******************/
	...
	public function order_summary()
	{
		...
		$this->load->model('account/subuser');
		$jsons = $this->model_account_subuser->getorder_summary($uid,$data);
		//$log->write($jsons);

		foreach ($jsons as $result) 
		{
			$growername=explode('-',$result['payment_firstname']);

			$data['OrderSummary'][] = array(
				'invoice_no' => $mcrypt->encrypt($result['order_id']),
				'grower_name' => $mcrypt->encrypt($growername[0]),
				'cash' => $mcrypt->encrypt($this->currency->format( round( $result['cash'],2))), 
				'tagged' => $mcrypt->encrypt($this->currency->format( round($result['tagged'],2))), 
				'subsidy' => $mcrypt->encrypt($this->currency->format( round($result['subsidy'],2))),
				'date' => $mcrypt->encrypt($result['dat']) 
			);
		}
		$this->response->setOutput(json_encode($data));
	...
	public function order_detail()
	{
		...
		$uid=$mcrypt->decrypt($this->request->post['username']);
		$data['order_id']=$mcrypt->decrypt($this->request->post['order_id']);

		$log->write($data);
		$this->load->model('account/subuser');
		$results=$this->model_account_subuser->getorder_summarydetail($data); 
		//$log->write($results); //print_r($results);
		foreach ($results as $result) 
		{
			$data1['OrderSummaryDetail'][] = array(
				'product_name' => $mcrypt->encrypt($result['name']),
				'quantity' =>$mcrypt->encrypt($result['quantity']),
				'price' =>$mcrypt->encrypt( $this->currency->format( round(($result['total']+($result['quantity']*$result['tax'])),2)) ), 
...
///////////metarial return ////////////
	...
	$mcrypt=new MCrypt();	
	$this->adminmodel('setting/setting');
	$log=new Log("subuser-getStoreUser-".date('Y-m-d').".log");	
	$log->write('getStoreUser called '); 
	$log->write($this->request->post); 
	$this->load->model('account/subuser');
	
	$data['store_id']=$mcrypt->decrypt($this->request->post['store_id']); 
	$log->write($data['store_id']);    
	$jsons =$this->model_account_subuser->getStoreUserlist($data);
			$log->write($jsons);
			foreach ($jsons as $ids) 
			{
				$log->write($ids);
	...
	public function return_order_trans()
	{
		$log=new Log("subuser_return_order-".date('Y-m-d').".log");
		$log->write("return_order_trans CALL");
		$log->write($this->request->post);
		 $mcrypt=new MCrypt();
		$data['products'] = ($_POST['product']);
		$data['prices'] = ($_POST['prices']);	
		...
		$data['imei']=$mcrypt->decrypt($_POST['imei']);
		$data['transid']=$mcrypt->decrypt($_POST['transid']);
		
		//$this->load->library('user');
        //$this->user = new User($this->registry);
		//$log->write("rk 1");
		$log->write($data);//$_POST);
		$this->load->model('account/subuser');	
		$datatrans=array();
		...
		foreach($data['stores'] as $store)
			...
			$query_return=$this->model_account_subuser->insert_subuser_return_products_otp_trans($datatrans);
			...
			if(!empty($query_return))
			{
				...
				$log->write($ids);
				if(strlen(trim($ids['username']))==10)
				{
					$mobile=$ids['username'];//'9911427348';// '7050101560';
				}
				else
				...
				{
					$json['success']="";
					$json['error'] = ('Please check Registered Mobile number of Store In-Charge');
					$log->write('Please check Registered Mobile number of Store In-Charge');
					$this->response->setOutput(json_encode($json));
					return;
				}
				$log->write($mobile); 
				...
				$json['success'] = $mcrypt->encrypt('Success: new order placed with ID: '.$query_return);						
				   ...
				   $json['error'] = ('Error in submission');
					...
					$json['success']="";
				   $json['error'] = ('Only One item can be returned at a time'); 
			}
			$this->response->setOutput(json_encode($json));
	...
	if(!empty($chekotp) && !empty($data['otp']))
	{
		if($chekotp==$data['otp'])
		{
			...
			foreach($data['products'] as $product)
			{
				...
				$qnty_check1=$this->model_account_subuser->check_current_qnty($data);
				$log->write('qnty_check');
				$log->write($qnty_check1);
				$log->write($data['quantity']);
				//if($qnty_check1['bal']>=$data['quantity'])
				//{
					$query_return=$this->model_account_subuser->insert_subuser_return_products($data);	
					$query_ret=$this->model_account_subuser->update_oc_contractor_product_otp_trans_status($data);
				//}
				//else
				//{
					//$json['success']="";
					//$json['error'] = "You don't have enough quantity of ".$data['product_name'];
					//$this->response->setOutput(json_encode($json));
					//return;
				//}			
			}
