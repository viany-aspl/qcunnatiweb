...
class ControllerMposMaterialRequest extends Controller{
    ...
    public function adminmodel($model) {
      ...
      if (file_exists($file)) {
		...
		$log=new Log("inv-orderlist-materialrequest-".date('Y-m-d').".log");
		$start = ($page-1)*20;
		$limit = 20;
		...
		foreach($da_data as $d_data)
		{
                    ...
                    $get_store_name=$this->model_materialrequest_purchase_order->get_store_name($store_id);
...
public function orderlistAll()
    {
        ...
        $log=new Log("inv-material-request-all".date('Y-m-d').".log");
        $start = ($page-1)*20;
        $limit = 20;
...
/*----------------------------view_order_details function starts here------------*/
	...
	{
				 $mcrypt=new MCrypt();
		$log=new Log("material-request-receive".date('Y-m-d').".log");
	...
	public function receive_order()
	{
		...
		$mcrypt=new MCrypt();
		$log=new Log("receive-marerial-request-product-".date('Y-m-d').".log");
		$this->adminmodel('materialrequest/purchase_order');
	...
	/*--------------------Insert Purchase Order starts heres-------------------------------------------------*/
            ...
            public function request_order()
            {
                   ...
                   $price=array();
                    ...
                    $log->write($data);
                    $this->session->data['user_id']=$user_id=$mcrypt->decrypt($_POST['username']);
                    $store_id= $mcrypt->decrypt($_POST['pstore']);
                    $log->write($store_id);
                    $log->write('Load admin modal start');
                    $this->adminmodel('materialrequest/purchase_order'); 
                    ...
                    //$data['options'] =$data['option_values']= array();
...
//                                    }                                     
                                   ...
                                   $log->write("prd id-".$mcrypt->decrypt($productval[0]));
                                    ...
                                    // $data['price']=$product_pack_size[0]['store_price'];
                                    //check for old purchase order is pending or not
                                      $pending_orders = $this->model_materialrequest_purchase_order->getList_pending_order($store_id,$mcrypt->decrypt($productval[0]));
...
//                                     }
                                    ...
                                    if(!empty($creditdata) && is_array($creditdata)){
...
//                                            }
	...
	///////////////////////////////////////////////////////////////////////////////////
        ...
        {                           
                ...
                $this->adminmodel('materialrequest/purchase_order');
		$data['order_information'] =$this->model_materialrequest_purchase_order->change_order_status($order_id,$order_status);	
		$log->write($data['order_information']);		
