...
class ModelCatalogProduct extends Model {
	...
	group by product_id,oc_order.user_id
		...
		$log->write('getMaterialSummary called in model for');
		$log->write($data);
		$log->write($sql);			
            return $query->row;    
	...
	public function getProduct($product_id) 
	{
		...
		$log->write('getProduct called in model for');
		$log->write($product_id);
$sql="SELECT DISTINCT *, pd.name AS name,pd.meta_hindi as meta_hindi, p.image, m.name AS manufacturer,
(SELECT MAX(subsidy) FROM " . DB_PREFIX . "product_subsidy pds WHERE pds.category_id <> 'null' and pds.product_id = p.product_id AND pds.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pds.quantity = '1' AND pds.store_id='".(int)$this->config->get('config_store_id')."'  ORDER BY pds.priority ASC LIMIT 1) AS subsidy , 
...
p.sort_order,p2s.quantity as squantity,
IFNULL((select quantity from " . DB_PREFIX . "product_to_store_field where product_id = p.product_id and store_id='" . (int)$this->config->get('config_store_id') . "' LIMIT 1 ),0) as fquantity,  
 IFNULL((select sum(quantity) as quantity from " . DB_PREFIX . "contractor_product where product_id = p.product_id and store_id='" . (int)$this->config->get('config_store_id') . "' group by oc_contractor_product.product_id LIMIT 1 ),0) as cquantity  
 FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
//if($this->config->get('config_store_id')==18)
{
	$log->write($sql);
}
		...
		if ($query->num_rows) {
			return array(
				...
				'meta_hindi'    => $query->row['meta_hindi'],
...
public function getProducts($data = array()) 
{
		...
		$log->write("str in 19 ".$storeid);
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW()  AND p2s.store_price >0  AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'"; 
		...
		{
			$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "contractor_product p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.quantity > 0 AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		}
		...
		{	
			...
			$prddd=$this->getProduct($result['product_id']);
			
		    $product_data[$result['product_id']] = $prddd;
			//data
			if(!empty($data['servicetype']))
			{
				$log->write("in material");
		 		$cdata=array();
		 		$cdata['filter_store'] =(int)$this->config->get('config_store_id');
		 		$cdata['filter_product']=$result['product_id'];
				$cdata['filter_user']=$data['uid']; 
				//$log->write("in material");
				//$log->write($cdata);
		 		$v=$this->getMaterialSummary($cdata)['bal'];
				$log->write($v);
				$product_data[$result['product_id']]['cquantity']=$v;
			 	//$log->write($v);
				//end data
			}	
...
public function getloanProduct($product_id) {
		...
		$sql="SELECT * FROM " . DB_PREFIX . "contractor_product  where product_id = '" . (int)$product_id . "' and  store_id = '" . (int)$this->config->get('config_store_id') . "'";
		$query = $this->db->query( $sql);
	...
	public function getloanProducts($data = array()) {

		$log=new Log("loan-".date('Y-m-d').".log");
		$log->write($data);
		$product_data = array();
		$sql="SELECT * FROM " . DB_PREFIX . "contractor_product  where contractor_id='".$data["user"]."' and  store_id = '" . (int)$this->config->get('config_store_id') . "'";
		$log->write($sql);
		$query = $this->db->query($sql);
		foreach ($query->rows as $result) 
		{
			$log->write($result['product_id']);
			$product_data[$result['product_id']] = $this->getloanProduct($result['product_id']);
...
public function getTotalLoanInventoryAmount( $sid){

$log=new Log("slog.log");
$sql="select sum(qnty),sum(price),(sum(total)) as total from (
SELECT p.product_id,p.name,sum(p.quantity) as qnty,sum(REPLACE(price,'Rs.','')) as price ,(sum(p.quantity) * REPLACE(price,'Rs.','')+sum(p.quantity) * REPLACE(tax,'Rs.','')) as  total,
tax
FROM `oc_contractor_product` p 
 WHERE   p.quantity > 0 AND p.store_id = '".$sid."' 
