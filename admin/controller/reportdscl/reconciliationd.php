...
class ControllerReportbcmlReconciliationd extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/reconciliationd', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		foreach ($results as $result) {
                        ...
                        $grower_info = $this->model_report_reconciliation->getGrowerDetailsd($result['order_id']);
	  ...
	  $inv_no=ucwords(strtolower(@$farmer_info[3]));
                $company=ucwords(strtolower(@$farmer_info[4]));
			$data['orders'][] = array(
		...
		$pagination->url = $this->url->link('report/reconciliationd', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/reconciliationd.tpl', $data));
	}
function cmp($a, $b)
{
...
Checked and Submitted by : 
    ...
    { 
        ...
        $col = 0;
         ...
         $grower_info = $this->model_report_reconciliation->getGrowerDetailsd($data['order_id']);
         $farmer_info=explode('-', $grower_info);
         $grower_id=@$farmer_info[0];
         $farmer_name=ucwords(strtolower(@$farmer_info[1]));
         $father_name=ucwords(strtolower(@$farmer_info[2]));
    ...
    public function Create_bill_pdf_only_once() {
		...
		if (isset($this->request->post['selected'])) {
			...
			$orders[] = $this->request->get['order_id'];
				...
				{
					//print_r($file_a_array[0]["file_name"]);
...
//print_r($data['orders']); 
		...
		<img src="../image/letterhead_topline.png" style="height: 10px; margin-left: -16px;width: 120%;" /> 
                ...
                $filename='tagged_order_'.$filter_date_start.'_'.$filter_date_end.'_'.$filter_store.'.pdf';     
                $mpdf->Output(DIR_UPLOAD.'tagged_pdf/'.$filename,'F');
				$order_id_array=$this->model_report_reconciliation->add_file_data_tagged($filter_data,$filename);
                //$mpdf->Output($filename,'D');
	        //$this->response->setOutput($this->load->view('tag/order_invoice.tpl', $data));
				}
