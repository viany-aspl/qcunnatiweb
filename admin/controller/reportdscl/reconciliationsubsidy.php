...
class ControllerReportdsclReconciliationsubsidy extends Controller 
{
	public function submit_order_to_dscl()
	{
		$log=new Log("submit_order_to_dscl_pink-".date('Y-m-d').".log");
		$order_id=$this->request->get['order_id'];
		$dscl_upload=$this->request->get['dscl_upload'];
		$this->load->model('sale/order');
		$this->load->model('pos/dscl');
		$order_info=$this->model_sale_order->getorder($order_id);
		
		$order_info['amtcash']=$order_info['cash'];
		$order_info['tagged_amt']=$order_info['tagged'];
		$order_info['sub']=$order_info['subsidy'];
		$order_info['subsidy_coupon']=$order_info['subsidy_form_no'];
		$order_info['oid'] = $order_id;//mt_rand(100000000,999999999);
		$order_info['order_trans_id']=mt_rand(100000000,999999999).$order_id;
		$order_info['otpu']='123456';
		$order_info['Card_Serial_Number']='0';
		$order_info['grower_id']=$order_info['shipping_firstname'];
		$order_info['vill']=substr($order_info['shipping_firstname'],0,4);
		$order_info['villname']=$order_info['vill'];
		$order_info['CARD_UNIT']=$order_info['payment_address_1'];
		$order_info['TAGGEDRATIO']=1;
		$order_info['order_product']=$this->model_sale_order->getOrder_detail($order_id);
		$order_info['order_product_subsidy']=$this->model_sale_order->getOrder_detail_subsidy_product($order_id);
		$order_info['order_total']=$this->model_sale_order->getOrder_detail_total($order_id);
		//print_r($order_info);
		if($order_info['payment_method']=='Subsidy')
		{
			$order_info['payment_method']='Cash Subsidy';
		}
		$order_info['old_data']='yes';
		$log->write($order_info);
		if(count($order_info['order_product'])>1)
		{
			$log->write('in if product count is 1');
			echo '2';
			exit;
		}
		if($dscl_upload=='2')
		{
			$UpdateDelivery=$order_info['dscl_trans_id'];
			$log->write('in if dscl_upload 2');
		}
		else
		{
			$UpdateDelivery=$this->model_pos_dscl->UpdateDelivery('UpdateDelivery',$order_info,0);
		}
		$log->write('UpdateDelivery - '.$UpdateDelivery);
		$UpdateDelivery=(int)($UpdateDelivery);
		$log->write('UpdateDelivery - '.$UpdateDelivery);
		
		if ((!empty($UpdateDelivery)) && (is_integer($UpdateDelivery)))
		{
			$order_info['oid']=$UpdateDelivery;//.$order_id;
			$order_info['billno']=$order_id;
			$order_info['billnofordscl']=$order_info['invoice_prefix'].$order_info['invoice_no'];
		
			$UpdateStatus=$this->model_pos_dscl->UpdateStatus('UpdateStatus',$order_info,0);
			$UpdateStatus=(int)($UpdateStatus);
			$log->write('UpdateStatus - '.$UpdateStatus);			
			if ((!empty($UpdateStatus)) && (is_integer($UpdateStatus)))
			{
				$updatestatus=$this->model_sale_order->updateDsclDeliverStatus($order_id,'1',$UpdateDelivery);
				echo '1';
				exit;
			}
			else
			{
				$updatestatus=$this->model_sale_order->updateDsclDeliverStatus($order_id,'2',$UpdateDelivery);
				echo '0';
				exit;
			}
		}
	...
	public function index() 
	{
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportdscl/reconciliationsubsidy', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
	...
	$farmer_name=$result['o_payment_address_1'];
	  ...
	  $inv_no=$result['requisition_id'];//ucwords(strtolower(@$farmer_info[3]));
		...
		if(empty($unit_name))
		{
			$unit_name=$result['unit_name_subsidy'];
		}
	$o_payment_address_1=$result['o_payment_address_1'];
		...
		$pagination->url = $this->url->link('reportdscl/reconciliationsubsidy', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportdscl/reconciliation_subsidy.tpl', $data));
	}
	public function subsidy() 
	{
		$this->load->language('report/reconciliation');

		$this->document->setTitle('Reconciliation Subsidy -Pink Purchy');
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('reportdscl/reconciliationsubsidy/subsidy', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
	...
	$o_payment_address_1=$result['o_payment_address_1'];
		  ...
		  'order_id'   => $inv_no ,
			'inv_no'   => $result['order_id'],
								...
								'father_name'=>$father_name,
								'o_bill_no'=> $result['bill_no'],
								'dscl_upload'=>$result['dscl_upload']
				
			);
			}
		}
		$data['heading_title'] = 'Reconciliation Subsidy - Pink Purchy';
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('reportdscl/reconciliationsubsidy/subsidy', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportdscl/reconciliation_subsidy_pink_purchy.tpl', $data));
	}
function cmp($a, $b)
{
...
Checked and Submitted by : 
		...
		public function download_pdf_subsidy() { 
            $this->load->language('report/reconciliation');
	...
	foreach ($results as $result) 
{
	...
	$orderproductsstring='';
		...
		foreach($orderproducts as $orderproduct) 
		{
				...
				$orderproductsstring=$orderproductsstring.$orderproduct['name'].' - ('.$orderproduct['quantity'].')-('.$orderproduct['discount_value'].'),';
...
$col = 0;
	...
	$o_payment_address_1=$result['o_payment_address_1'];
		...
		$fdata=array(
							...
							number_format((float)$result['total'], 2, '.', ''),
                            number_format((float)$taggedvalue, 2, '.', ''),
                            number_format((float)$result['subsidy'], 2, '.', ''),
							number_format((float)$result['cash'], 2, '.', ''),
							$data['subsidy_form_no'],
							$data['subsidy_category'],
							$data['subsidy_category_name'],
							$orderproductsstring
                            );
			 fputcsv($fileIO,  $fdata,",");
...
*/
 $grower_info = $data['payment_address_1'];//$this->model_report_reconciliation->getGrowerDetails($result['order_id']);
	...
	{ 
		$log=new Log("report-dscl-create-bill-".date('Y-m-d').".log");		
		$log->write('create_bill called for subsidy');
            $this->load->language('report/reconciliation');
		...
		$filter_data = array(
			...
			'bill_type'=>'subsidy'
                            ...
                            $file_a_array=$this->model_report_reconciliation->get_file_data_subsidy($filter_data);
                            $file_in_database=""; 
		$file_server= "";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   ...
	   else
	   {
	        ...
	        $data["file_aspl"]=$this->model_report_reconciliation->add_file_data_subsidy($filter_data,$filename);   
	   }
		...
		$cardsql="select  order_id  from oc_order_req_delivery where payment_method in ('Tagged Subsidy','Tagged Cash Subsidy') and  EMAIL!='tech@em3agri.com' and EMAIL!='TRRINGO.COM' and  EMAIL!='".strtoupper("tech@em3agri.com")."' and  UNIT_CODE='".$filter_unit."' and TO_CHAR(DATE_ADDED,'YYYY-MM-dd')='".$filter_date."' and ORDER_STATUS_ID='5' ";
		$log->write($cardsql);
		...
		foreach($dsclfinaldata as $dscldata)
		{
			if(!in_array($dscldata,$akshfinaldata))
			{
				$error_order=$error_order.$dscldata.',';
...
$col = 0;
	...
	$o_payment_address_1=$result['o_payment_address_1'];
		...
		$data['start_date'] = date($this->language->get('date_format_short'), strtotime($filter_date));
                ...
                $html = $this->load->view('reportdscl/reconciliation_pdf_subsidy.tpl',$data);
                //exit;
...
<img src="../image/letterhead_topline.png" style="height: 10px; margin-left: -46px;width: 120% !important;" /> 
	  ...
	  $update_total=$this->model_report_reconciliation->update_file_data_subsidy($filter_data,$data["file_aspl"],$total_amount,$logged_user,$filename); 
                $mpdf->Output($filename,'D');
	  header('location: '.$_SERVER['HTTP_REFERER']);
	...
	public function create_bill_subsidy() 
	{ 
		$log=new Log("report-dscl-create-bill-".date('Y-m-d').".log");		
		$log->write('create_bill called for subsidy');
            $this->load->language('report/reconciliation');
