...
class ControllerReportdsclCashReport extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportdscl/cash_report', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$pagination->url = $this->url->link('reportdscl/cash_report', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$data["stores"]=$this->model_setting_store->getStores();//$this->model_setting_store->getStoresCompanyWise($get_company_id);
		...
		$data['allces']=$this->model_runner_cash->getCeByCompany(array('filter_company'=>$get_company_id));
		//print_r($data['allces']);
		...
		$this->response->setOutput($this->load->view('reportdscl/cash_report.tpl', $data));
	}
...
public function download_excel_runner() {
	...
	$get_company_id='1';
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportdscl/cash_report/cash_position', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$filter_data = array(
			...
			'limit'                  => $this->config->get('config_limit_admin')
                ...
                $arr=$this->model_report_cash->getTotalCash_position_CompanyWise($filter_data);
	  $order_total = $arr["total"];
                $data["total_amount"] =$this->model_report_cash->get_own_stores_total_eod_company($filter_data)["total_amount"]; //$arr["total_amount"];
                
		$data['orders'] = array(); 

		$results = $this->model_report_cash->getCash_position_CompanyWise($filter_data);
		...
		$this->load->model('setting/store');
		$data["stores"]=$this->model_setting_store->getStoresCompanyWise($get_company_id);
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('reportdscl/cash_report/cash_position', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
                $this->response->setOutput($this->load->view('reportdscl/cash_position.tpl', $data));
        }
...
if($data['bank_id']=="6")
	...
	$get_company_id='1';
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportdscl/cash_report/runner_cash_position', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$filter_data = array(
			...
			'limit'                  => $this->config->get('config_limit_admin')
                ...
                $arr=$this->model_runner_cash->get_runner_total_Cash_position_companywise($filter_data);
	   //echo "here";
                 //print_r($arr);
		...
		$results = $this->model_runner_cash->get_runner_Cash_position_companywise($filter_data);
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('reportdscl/cash_report/runner_cash_position', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($order_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($order_total - $this->config->get('config_limit_admin'))) ? $order_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $order_total, ceil($order_total / $this->config->get('config_limit_admin')));
		$data['runner_list']=$this->model_runner_cash->getAllCe(array('filter_company'=>1));
		$data['filter_user'] = $filter_user;
                         
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
                $this->response->setOutput($this->load->view('reportdscl/ce_current_report.tpl', $data));
        }
