...
global $reg;
class ControllerReportdsclReport extends Controller {
   ...
   private function download_excel_2($fields,$fdatas,$filename)
	...
	public function tagged_amount_download_excel() 
	{
		
		$heading='Tagged Amount';
		
	
		if (isset($this->request->get['filter_unit'])) 
		{
			$filter_unit =$data['filter_unit']= $this->request->get['filter_unit'];
        ...
        $get_stores=$this->model_report_reconciliation->tagged_amount($filter_data);
		...
		foreach ($get_stores as $result)
        {
				//echo date('Y-m-d',strtotime($result['date_start']));
            $fdata[]=array(
                ($result['letter_no']),
                $result['unit_name'],
                date('Y-m-d',strtotime($result['date_start'])),
		...
		$this->download_excel_2($fields,$fdata,$heading.'-');
	}
	public function tagged_amount() 
	{
		...
		$data['breadcrumbs'][] = array(
			'text' => $heading,
			'href' => $this->url->link('reportdscl/report/tagged_amount', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
        ...
        $get_stores=$this->model_report_reconciliation->tagged_amount($filter_data);
		//print_r($get_stores);
		$order_total = $this->model_report_reconciliation->tagged_amountTotal($filter_data);
		
		foreach($get_stores as $stores)
		{  
			$data['orders'][] = array(
				'total_amount'          => $stores['total_amount'],
				'unit_name'    => $stores['unit_name'],
				'store_name'    => $stores['store_name'],
				'date_start'    => $stores['date_start'],
				'letter_no'    => $stores['letter_no']
		...
		$pagination->url = $this->url->link('reportdscl/report/tagged_amount', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportdscl/tagged_amount.tpl', $data)); 
	}
	...
	public function index() {
       ...
       $today_cash=$this->model_report_dscl_input_sales->getcash($stores['store_id'],$filter_date,$stores['unit_id']);
       $today_tagged=$this->model_report_dscl_input_sales->gettagged($stores['store_id'],$filter_date,$stores['unit_id']);
       
       
       $till_date_cash=$this->model_report_dscl_input_sales->getcashtilldate($stores['store_id'],$filter_date,$stores['unit_id']);
       $till_date_tagged=$this->model_report_dscl_input_sales->gettaggedtilldate($stores['store_id'],$filter_date,$stores['unit_id']);
	   ...
	   $data['orders'][] = array(
        ...
        public function download_dscl_sales_report() {
