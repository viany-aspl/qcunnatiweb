...
class ControllerreportdsclBillsubmission extends Controller {
	private $error = array();
	...
	public function index() { //print_r($this->request->post);exit;
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportdscl/billsubmission/getlist', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$pagination->url = $this->url->link('reportdscl/billsubmission/getlist', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportdscl/bill_list_view.tpl', $data));
