...
error_reporting(0);
class ControllerIsecPurchaseOrder extends Controller{
	...
	public function index()
	{
		//set the title of the page
			$this->document->setTitle("ISEC Requisition List");
			...
			$data['breadcrumbs'][] = array(
			'text' => "ISEC Requisition",
			'href' => $this->url->link('isec/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('isec/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('isec/purchase_order/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['filter'] = $this->url->link('isec/purchase_order/filter', 'token=' . $this->session->data['token'] . $url, true);
		...
		$order_list = $this->model_purchase_purchase_order->isecgetList($filter_data);
		//$data['order_list']=array();
                ...
                foreach($order_list as $order)
                {
                    ...
                    $order_id=$order['id'];
                     $order_information= $this->model_purchase_purchase_order->view_order_details($order_id);
		...
		//print_r($order_list);
		$total_orders = $this->model_purchase_purchase_order->isecgetTotalOrders($filter_data);
		
		//getting total orders
		$data['view'] = $this->url->link('isec/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		$data['receive'] = $this->url->link('isec/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
		...
		$pagination->url = $this->url->link('isec/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('isec/purchase_order_list.tpl', $data));
	}
	...
	public function view_order_details()
	{
		...
		//print_r($data['order_information']);
                $data['cancel'] = $this->url->link('isec/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['pdf_export'] = $this->url->link('isec/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		...
		{
			$this->response->setOutput($this->load->view('isec/view_order.tpl',$data));
		}
	}
	...
	public function receive_order()
	{
			...
			if (isset($this->request->get['page'])) {
                ...
                if($this->request->server['REQUEST_METHOD'] == 'POST')
                {
                    ...
                    $data['order_receive_date']=$this->request->post['order_receive_date'];
                       $inserted = $this->model_purchase_purchase_order->insert_receive_order($this->request->post,$order_id);
                  $this->session->data['success'] = 'Order Accepeted successfully';
                    $this->response->redirect($this->url->link('isec/purchase_order', 'token=' . $this->session->data['token'] . $url, 'SSL'));
            //print_r($this->request->pst);exit;
                }
		...
		$data['action'] = $this->url->link('isec/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link('isec/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('isec/receive_order.tpl',$data));
	...
	/*--------------------Insert Purchase Indent starts heres-------------------------------------------------*/
