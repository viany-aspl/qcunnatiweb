...
class ControllerSaleOrder extends Controller {
	private $error = array();
	public function set_subsidy_cash()
	{
		$this->load->model('sale/order');
		$filter_data = array(
			'filter_user_id' => '',
			'filter_order_id'      => '',
			'filter_requistion_id'=>'',
			'filter_payment'=>'Subsidy',
			'filter_customer'	   => '',
			'filter_order_status'  => 5,
			'filter_total'         => '',
			'filter_date_added'    => '2018-11-01',
			'filter_date_modified' => '2018-11-25',
			'filter_store'=>'',
			'sort'                 => '',
			'order'                => '',
			'start'                => 0,
		...
		//print_r($results);
		$log=new Log("set_subsidy_cash-".date('Y-m-d').".log");
		...
		{
			$log->write('');
			$log->write($result['order_id']);
			$log->write($result['payment_method']);
			$log->write($result['cash']);
			$sql = "update " . DB_PREFIX . "user set cash = cash + ".$result['cash']." where user_id = ".$result['user_id'];
			...
			{ 
				...
				//$this->db->query("delete from oc_store_cash_trans where order_id=".$result["order_id"]." and payment_method='Subsidy' and tr_type='CR' and user_id='".$result['user_id']."' ");
                //$trans->addstoretrans($result['cash'],$result['store_id'],$result['user_id'],'CR',$result["order_id"],$result['payment_method'],$result['total'],'','5',$result['date_added']);  
            } 
			catch (Exception $e)
			{
