...
private static function compare_tax_rates ($a, $b) {
	...
	public function productbarred() {
		
		$this->document->setTitle("Product Barred");
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->model_catalog_product->updatebarredproducts($this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/product/productbarred', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}

		$this->getbarredForm();
	}
	
	protected function getbarredForm() {
		...
		$results = $this->model_catalog_product->getProductsBarred();
              
		foreach ($results as $result) { //print_r($result);
			

			$data['barredproducts'][] = array(
		...
		$data['products'] = $this->model_catalog_product->getProductss();
		$data['menus'] = $this->model_catalog_storemenu->getmenu();
		...
		$this->response->setOutput($this->load->view('catalog/productbarred_form.tpl', $data));
	}
	public function add() {
		$this->load->language('catalog/product');
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'edit'       => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL'),
                                'product_credit'       => $this->url->link('catalog/store_credit/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL'),
