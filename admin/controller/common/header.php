...
/*        $pos_user_group_id = $this->config->get('pos_user_group_id');
            ...
            if(isset($user['user_group_id']) && ($pos_user_group_id == $user['user_group_id']) && $has_permission==1)
	{
			...
			// Processing Orders
			$data['order_status_total'] = array();//$this->model_sale_order->getTotalOrders(array('filter_order_status' => implode(',', $this->config->get('config_processing_status'))));
			...
			// Complete Orders
			$data['complete_status_total'] = array();//$this->model_sale_order->getTotalOrders(array('filter_order_status' => implode(',', $this->config->get('config_complete_status'))));
			...
			$return_total = array();//$this->model_sale_return->getTotalReturns(array('filter_return_status_id' => $this->config->get('config_return_status_id')));
			...
			$data['online_total'] = array();//$this->model_report_customer->getTotalCustomersOnline();

			$data['online'] = $this->url->link('report/customer_online', 'token=' . $this->session->data['token'], 'SSL');

			$this->load->model('sale/customer');

			$customer_total = array();//$this->model_sale_customer->getTotalCustomers(array('filter_approved' => false));
			...
			$product_total = array();//$this->model_catalog_product->getTotalProducts(array('filter_quantity' => 0));
			...
			$review_total =array();// $this->model_catalog_review->getTotalReviews(array('filter_status' => false));
			...
			$affiliate_total =array();// $this->model_marketing_affiliate->getTotalAffiliates(array('filter_approved' => false));
