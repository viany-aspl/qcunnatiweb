...
class ControllerReportbcmlbcmlcash extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->document->setTitle($this->language->get('BCML Cash'));

		$this->load->model('report/bcml_cash');
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('BCML Cash'),
			'href' => $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['invoice'] = $this->url->link('reportbcml/bcml_cash/invoice', 'token=' . $this->session->data['token'], 'SSL');
		$data['shipping'] = $this->url->link('reportbcml/bcml_cash/shipping', 'token=' . $this->session->data['token'], 'SSL');
		$data['insert'] = $this->url->link('reportbcml/bcml_cash/add', 'token=' . $this->session->data['token'], 'SSL');
...
$filter_data = array(
		...
		$order_total = $this->model_report_bcml_cash->getTotalOrders($filter_data);

		$results = $this->model_report_bcml_cash->getOrders($filter_data);
      //  print_r(unserialize($results[0]['response']);
		//exit;
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'view'          => $this->url->link('reportbcml/bcml_cash/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'edit'          => $this->url->link('reportbcml/bcml_cash/edit', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'delete'        => $this->url->link('reportbcml/bcml_cash/delete', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
		}

		$data['heading_title'] = $this->language->get('BCML Cash');
		...
		$data['sort_order'] = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . '&sort=o.order_id' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_total'] = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . '&sort=o.total' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . '&sort=o.date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . '&sort=o.date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('reportbcml/bcml_cash', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$data['payment_methods']=array('Tagged','Tagged Cash','Cash','Subsidy','Cash Subsidy','Tagged Subsidy');
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('reportbcml/bcml_cash_report.tpl', $data));
	}
     
	    public function submit_remark()
        {
	 
	     $log=new Log("bcml-cash-submit-".date('Y-m-d').".log");
         $this->load->model('pos/bcml');
         $this->load->model('report/bcml_cash');
		
		 
		 
		 $result= $this->model_report_bcml_cash->getOrder($this->request->post['order_id']);
		 	$log->write("result");		
	         $log->write($result);
		    $product= $this->model_report_bcml_cash->getOrderProducts($this->request->post['order_id']);
		    $log->write("result1");		
	         $log->write($product);
                $data['vname']=($this->request->post['vname']);
				$data['gmobile']=$result['telephone'];
				$data['gname']=$result['firstname'];
				$data['gcode']=$result['shipping_firstname'];
				$data['invoicevalue']=$result['total'];
				
				$data['FmCode']=($result['fmcode']);
				$data['DeliveryMode']='Cash';
				
				$log->write("checkdata");		
	            $log->write($data);	
				//[{"S_DESC":"","actamount":"1780.00000","product_hstn":"0000","product_tax":"54.31",
				//"subsidyamount":"0.00000","product_price":"301.69","product_name":"CHLORO GRANULE 4 KG",
				//"subrate":"356.00","product_quantity":"5","bcml_code":"9428","total":"1780.00000","actrate":"356.00",
				//"product_id":"428","subsidyper":"0.00","SN":"1"}]
				//$data['prddtl']= json_decode($product['ActAmount'],TRUE);
				$data['product_hstn']= ($product['hsn_code']);
				$data['product_tax']= ($product['tax']);
				$data['subsidyamount']= ($product['SubsidyAmount']);
				$data['product_price']= ($product['price']);
				$data['product_name']= ($product['name']);
				$data['subrate']= ($product['SubRate']);
				$data['product_quantity']= ($product['quantity']);
				$data['bcml_code']=($product['BCMLCODE']);
				$data['total']= ($product['total']);
				$data['actrate']= ($product['ActRate']);
				$data['product_id']= ($product['product_id']);
				$data['subsidyper']= ($product['SubSidyPer']);
				$data['SN']= ($product['SN']);
				$log->write("checkdata2");		
	            $log->write($data);
			...
			/*	foreach ($it as $key=>$val)
				...
				$data['unitid']=empty($result['unit_id'])?$result['unit_id']:$result['unit_id'];
				$data['storeid']=$result['store_id'];
				$data['glimit']=0;
				$data['userid']=$result['user_id'];
				$data['billno']=$this->request->post['order_id'];
				$data['otp']=0;
				...
				//$this->model_pos_pos->insert_cash_order_trans($order_id,$data['store_id'],$data,$cashreturn);
		        //print_r($data);
		 
		     $log->write("checkdata3");		
	         $log->write($data);	
             $res= $this->model_pos_bcml->setOrderDataToServer($data,'Cash');
		     $log->write("submit");		
	         $log->write($res);	
				 if($res!=1) 
				{
		...
		return $res;
              /*  if($res)
                {
                $this->session->data['success'] = 'Insert Successfully';
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['text_tagged'] = "Tagged:";//$this->language->get('text_telephone');
    ...
    { 
		...
		foreach($orderinfos as $orderinfo)
		{
			...
			$fdata=array(
                            ...
                            $data['order_id'],
                            $data['payment_method'],
                            date('Y-m-d',strtotime($data['date_added'])),
                            $orderinfo['name'],
                            $orderinfo['quantity'],
                            $orderinfo['price'],
                            $orderinfo['tax'],
    ...
    header('Content-Disposition: attachment;filename="orders_itemized_report_'.date('dMy').'.csv"');
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $data['customer']);
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(12, $row, $data['shipping_firstname']);
        $row++;
	}
    ...
    header('Content-Disposition: attachment;filename="orders_itemized_report_'.date('dMy').'.xls"');
    header('Cache-Control: max-age=0');
	...
	*/
    ...
    { 
		...
		foreach($orderinfos as $orderinfo)
		{
			$orderprd=$orderprd.$orderinfo['name'].'('.$orderinfo['quantity'].'), ';
		}
			$orderprd=rtrim($orderprd,',');
			...
			$fdata=array(
							...
							number_format((float)($data['tagged']),2,'.',''),
	...
	fseek($fileIO, 0);
    header('Content-Type: application/csv');
    header('Content-Disposition: attachment;filename="orders_report_'.date('dMy').'.csv"');
    header('Cache-Control: max-age=0');
    fpassthru($fileIO);  
    fclose($fileIO);
