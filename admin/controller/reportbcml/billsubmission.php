...
class ControllerreportbcmlBillsubmission extends Controller {
	private $error = array();
	...
	public function index() { //print_r($this->request->post);exit;
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportbcml/billsubmission/getlist', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$pagination->url = $this->url->link('reportbcml/billsubmission/getlist', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportbcml/bill_list_view.tpl', $data));
