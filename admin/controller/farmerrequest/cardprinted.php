<?php
class ControllerFarmerrequestCardprinted extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->cardPrintedForm();
	}

          
        protected function cardPrintedForm() {
		              error_reporting(0);
...
if (isset($this->request->get['filter_villageid'])) {
		...
		if (isset($this->request->get['filter_association'])) {
		$filter_association=$this->request->get['filter_association'];
		}
		else
		{
			$filter_association='';
...
if (isset($this->request->get['filter_villageid'])) {
		...
		if (isset($this->request->get['filter_association'])) {
			$url .= '&filter_association=' . $this->request->get['filter_association'];
		}
		if (isset($this->request->get['filter_zone'])) {
...
$url .= '&filter_date_end=' . $this->request->get['filter_date_end'];
		...
		$filter_data = array(
			...
			'filter_association' => $filter_association,
		...
		if($filter_unit1!="" )
		{
		$results = $this->model_farmerrequest_farmerrequest->getprinteddtl($filter_data);
	   $order_total=$revieTotal = $this->model_farmerrequest_farmerrequest->getprintedToatal($filter_data);
	  // echo $order_total; 
	   }
 ...
 $selected='false'; 
		...
		$this->response->setOutput($this->load->view('farmerrequest/cardprintedform.tpl', $data));
	}
...
PHPExcel_Cell::setValueBinder(new PHPExcel_Cell_AdvancedValueBinder());
 ...
 $objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2, $row)->setValueExplicit($data['GROWER_ID'], PHPExcel_Cell_DataType::TYPE_STRING); 
		...
		//$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, $data['QR_SRTING']);
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, $data['ASSOCIATION']);
        $row++;
...
<img src="../image/letterhead_topline.png" style="height: 10px; margin-left: -46px;width: 120% !important;" /> 
    ...
    //$header='';
             ...
             $mpdf->setAutoTopMargin = 'stretch';
               ...
               $mpdf->WriteHTML($html);
				//$mpdf->WriteHTML('<p style="color:red;">hello rohit</p>');
