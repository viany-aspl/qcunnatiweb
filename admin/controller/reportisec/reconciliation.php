...
class ControllerReportisecReconciliation extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('reportisec/reconciliation', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
...
//$inv_no=$result['requisition_id'];
	  ...
	  $inv_no=$result['requisition_id'];//ucwords(strtolower(@$farmer_info[3]));
		...
		$pagination->url = $this->url->link('reportisec/reconciliation', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportisec/reconciliation.tpl', $data));
	}
function cmp($a, $b)
{
