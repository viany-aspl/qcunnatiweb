...
class ControllerReportisecSaleOrder extends Controller {
	public function index() {
		...
		$t1=$this->model_report_sale->getTotalOrdersCompanywise($filter_data);
		$order_total = $t1["total"];
		$No_Orders_all=$t1["orders"];	
                            $Total_amount_all=$t1["amount_total"];

		$results = $this->model_report_sale->getOrdersCompanywise($filter_data);
		$No_Orders=0;	
                            $Total_amount=0;
		...
		$pagination->url = $this->url->link('reportbcml/sale_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportbcml/sale_order.tpl', $data));
	}
