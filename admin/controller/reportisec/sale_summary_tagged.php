...
class ControllerReportisecSaleSummaryTagged extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Sale Summary',
			'href' => $this->url->link('reportisec/sale_summary_tagged', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$t1= $this->model_tag_sale_summary->getTotalSaleCompanyWise($filter_data);
		$order_total =$t1["total"];
		$total_cash_order_all=$t1["cash_order"]; $total_tagged_order_all=$t1["tagged_order"];

		$data['orders'] = array();

		$results = $this->model_tag_sale_summary->getSale_summaryCompanyWise($filter_data);
		...
		$pagination->url = $this->url->link('reportisec/sale_summary_tagged', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('reportisec/sale_summary_tagged.tpl', $data));
	}
...
if (isset($this->request->get['filter_date_start'])) {
        ...
        $this->load->model('tag/sale_summary');
        $order_total = $this->model_tag_sale_summary->getTotalSaleCompanyWise($filter_data);
