...
class ControllerReportisecReconciliationspray extends Controller {
	public function index() {
		$this->load->language('report/reconciliation');

		$this->document->setTitle('Reconciliation Report (Spray)');
		...
		$data['breadcrumbs'][] = array( 
			'text' => 'Reconciliation Report (Spray)',
			'href' => $this->url->link('reportisec/reconciliationspray', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$t1=$this->model_report_reconciliation->getTotalOrdersSprayCompanywise($filter_data);
		$order_total = $t1["total"];
		$total_tagged_amount_all=$t1["total_tagged_amount"];
		$total_tagged_amount=0;
		$results = $this->model_report_reconciliation->getOrdersSprayCompanywise($filter_data); 
	  ...
	  $inv_no=$result['requisition_id'];//ucwords(strtolower(@$farmer_info[3]));
		...
		$pagination->url = $this->url->link('reportisec/reconciliationspray', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$data['units']=$this->model_unit_unit->getunit(array('filter_company'=>'3'));
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('reportisec/reconciliation_spray.tpl', $data));
	}
function cmp($a, $b)
{
...
*/
    ...
    header('Content-Disposition: attachment;filename="spray_reconciliation_report_'.date('dMy').'.xls"');
...
*/
 $grower_info = $data['payment_address_1'];//$this->model_report_reconciliation->getGrowerDetails($result['order_id']);
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, number_format((float)($orderinfo['price']),2,'.','')); 
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, number_format((float)($orderinfo['tax']),2,'.',''));           
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   ...
	   {
                      $filename='spray_tagged_order_'.$filter_date.'_'.$filter_store.'_'.$filter_unit.'.pdf';  
	        $data["file_aspl"]=$this->model_report_reconciliation->add_file_data_tagged($filter_data,$filename);   
	   }
		$results = $this->model_report_reconciliation->getOrdersSpray($filter_data);
                         $total_amount=0;
		foreach ($results as $result) {
                        /*
