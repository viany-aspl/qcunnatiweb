...
class ControllerInvoiceAdjustment extends Controller{
		...
		if($filter_store!="")
        {
			$results = $this->model_invoice_adjustment->getList($filter_data);
		}
        
    header('Content-type: application/excel');
	$filename = 'partner_invoice_list.xls';
	...
	$html='<table class="table table-bordered table-hover">
		  ...
		  <td class="text-left">Invoice Amount</td>
            ...
            foreach($results as $order)
			{
				...
				else if($order['paid_status']=="1")
				{ 
					$paid_status="Paid";
				}
				if(($order['paid_date']!="0000-00-00") && ($order['paid_date']!=""))
				{ 
			...
			$html.='<tr>
                            ...
                            <td class="text-left">'.$order['po_invoice_prefix']."/".$order['po_invoice_n'].'</td>  
                            <td class="text-left">'.$order['order_total'].'</td> 
                            <td class="text-left">'.$order['create_date'].'</td>
                            <td class="text-left">'.$paid_status.'</td>
                            <td class="text-left">'.$paid_date.'</td>
