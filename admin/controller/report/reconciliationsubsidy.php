...
class ControllerReportReconciliationsubsidy extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
			...
			$grower_info =$this->model_report_reconciliation->getGrowerDetailsByNum($result['telephone']);//getGrowerDetails($result['order_id']);
			}
                        $farmer_info=explode('-', $grower_info);
      
                $grower_id=@$farmer_info[0];

			if(empty($result['grower_code'])){
			$result['grower_code']=	 $grower_id;
			}
	...
	if(!empty($result['shipping_company']))
	{
		...
		$company=$result['company'];
		  ...
		  'order_id'   => $inv_no ,
			...
			'Ref_no'  =>$result['refno'],
                                ...
                                'subsidy'     =>number_format((float)$result['subsidy'], 2, '.', ''),//$result['subsidy'] 
...
function cmp($a, $b)
{
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, $data['refno']);
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $row, number_format((float)$data['total'], 2, '.', ''));
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $row, number_format((float)$data['subsidy'], 2, '.', ''));
...
public function download_item_excel() {
	...
	foreach($orderinfos as $orderinfo){
		if($orderinfo['discount_value']!='')
		{
			$subsidy_percentage=$orderinfo['discount_value'];
			
			$SubsidyValue11=$this->model_report_reconciliation->getProductSubsidyValue($data['store_id'],$orderinfo['product_id']);
			$subsidy_category_name=$SubsidyValue11['category_name'];
		}
		if($orderinfo['discount_value']=='')
		{
			$SubsidyValue11=$this->model_report_reconciliation->getProductSubsidyValue($data['store_id'],$orderinfo['product_id']);
			$subsidy_percentage=$SubsidyValue11['subsidy'];
			$subsidy_category_name=$SubsidyValue11['category_name'];
		}
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(10, $row, $subsidy_percentage);	
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $subsidy_category_name);	
	...
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(13, $row,number_format((float)$data['subsidy'], 2, '.', ''));                
...
public function create_bill() { 
		...
		if (isset($this->request->post['filter_date22'])) {
			$filter_date22 = $this->request->post['filter_date22'];
		} else {
			$filter_date22= date('Y-m-d', strtotime(date('Y') . '-' . date('m') . '-01'));
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   ...
	   else
	   {
		...
		foreach ($results as $result) {
                ...
                $grower_id=$result['grower_code'];//@$data['grower_code'];
                $farmer_name=ucwords(strtolower(@$farmer_info[1]));
                $father_name=ucwords(strtolower(@$farmer_info[2]));
				$inv_no=$result['requisition_id'];//ucwords(strtolower(@$farmer_info[3]));
