...
class ControllerReportCe extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/ce', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$pagination->url = $this->url->link('report/ce', 'token=' . $this->session->data['token'] . $url . '&page1={page}&tab=tab1&page2='.$page2, 'SSL');
...
//////////////////////////////////////////////tab 2 start here//////////////////////////
		...
		$filter_data2 = array(
			...
			'limit'                  => $this->config->get('config_limit_admin')
                           ...
                           $order_total_records = $this->model_report_cash->getTotalCash_transationRunner($filter_data2);

		$bank_totals2 = $this->model_report_cash->getTotalCash_transationRunnerbank($filter_data2);
                            //print_r($bank_totals2 );
		$data['records'] = array();

		$results = $this->model_report_cash->getCash_reportRunner($filter_data2);
		...
		$pagination2->url = $this->url->link('report/ce', 'token=' . $this->session->data['token'] . $url . '&page2={page}&tab=tab2&page1='.$page1, 'SSL');
		...
		$data['tool_tip']=$this->language->get('report/cash_report_ce');
		...
		$this->response->setOutput($this->load->view('ce/cash_report.tpl', $data));
	}
