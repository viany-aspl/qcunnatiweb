...
class ControllerReportReconciliationspray extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/reconciliationspray', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$t1=$this->model_report_reconciliation->getTotalOrdersSpray($filter_data);
	  ...
	  $inv_no=$result['requisition_id'];//ucwords(strtolower(@$farmer_info[3]));
		...
		$pagination->url = $this->url->link('report/reconciliationspray', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$this->load->language('common/information');
		$data['tool_tip']=$this->language->get('report/reconciliationspray');
		...
		$this->response->setOutput($this->load->view('report/reconciliation_spray.tpl', $data));
	}
function cmp($a, $b)
{
...
*/
	  ...
	  $inv_no=$result['requisition_id'];//ucwords(strtolower(@$farmer_info[3]));
			...
			'inv_no'   => $result['order_id'],
                                ...
                                'store_id'   => $result['store_id'],
                                'total'      => number_format((float)$result['total'], 2, '.', ''),//$result['total'],
                                'tagged'     => number_format((float)$result['subsidy'], 2, '.', ''),//$result['tagged'],
                                'grower_id'  => $grower_id,
                                'farmer_name'=> $farmer_name,
                                'unit'       => $result['company']
