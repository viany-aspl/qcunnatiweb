...
class ControllerReportReconciliation extends Controller {
	public function index() {
		...
		$t1=$this->model_report_reconciliation->getTotalOrders($filter_data);
...
if(empty($result['payment_address_1']))
{
//$result['payment_address_1']=$result['o_payment_address_1'];
}
...
Checked and Submitted by : 
    ...
    { 
        ...
        $col = 0;
       if($data["order_id"]=="51248")
{
//$grower_info = "770100029 - HARISHARAN - RAM BHAROSEY"; 
//$inv_no="25436";
$data['order_id']="1090623";
//$result['company']="HARIYAWAN";
}else
{

//$grower_info = $result['payment_address_1'];
//$inv_no=$result['requisition_id'];
} 
...
public function create_bill_1() {
		...
		if (isset($this->request->get['filter_date_start'])) {
			$filter_date_start = $this->request->get['filter_date_start'];
                ...
                else if (isset($this->request->post['filter_date'])) {
			$filter_date_start = $this->request->post['filter_date'];
		}else {
			$filter_date_start = date('Y-m-d');
		}
		...
		if (isset($this->request->get['filter_date_end'])) {
			...
			$filter_date_end = $this->request->post['filter_date'];
		}
                else {
			$filter_date_end = date('Y-m-d');
		}
		...
		if (isset($this->request->get['filter_store'])) {
			$filter_store = $this->request->get['filter_store'];
                ...
                else if (isset($this->request->post['filter_store_2'])) {
		...
		if (isset($this->request->get['filter_unit'])) {
			$filter_unit = $this->request->get['filter_unit'];
                ...
                else if (isset($this->request->post['filter_unit_2'])) {
		...
		if (isset($this->request->get['filter_unit'])) {
			...
			$url .= '&filter_date_start=' . $this->request->post['filter_date'];
		}

		if (isset($this->request->post['filter_date'])) {
			$url .= '&filter_date_end=' . $this->request->post['filter_date'];
		}

		if (isset($this->request->post['filter_store_2'])) {
			$url .= '&filter_store=' . $this->request->post['filter_store_2'];
		}
		if (isset($this->request->post['filter_unit_2'])) {
			$url .= '&filter_unit=' . $this->request->post['filter_unit_2'];
		}
		...
		foreach ($results as $result) 
                {
                    if(in_array($result['order_id'],$_SESSION[session_id()]))
                    {
                        $selected='true';
                    }
                    else
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('report/reconciliation/create_bill_1', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/reconciliation_bill.tpl', $data));
	} 
    public function check_selected_orders_count()
    {
                if (isset($this->request->get['filter_date_start'])) {
			$filter_date_start = $this->request->get['filter_date_start'];
		} else {
		...
		$order_total = $t1["total"];
                ...
                {
                $selected_count=count($current_array);
                }
                else 
                {
                   $selected_count=0;
                   echo "Please Select the bills for Print";
                   return;
                }
                
                if($selected_count<$order_total)
                {
                    echo "Please Select all bills for Print";
                    return;  
                }
                else 
                {
    ...
    public function select_all_bills()
    {
		...
		foreach ($results as $result) 
                {
                    //print_r($result);
                    array_push($current_array,$result['order_id']); 
	...
	{ 
		...
		if (isset($this->request->post['filter_date'])) {
            $filter_date = $this->request->post['filter_date'];
                ...
                else if (isset($this->request->get['filter_date'])) {
            $filter_date = $this->request->get['filter_date'];
        } else {
            $filter_date= date('Y-m-d', strtotime(date('Y') . '-' . date('m') . '-01'));
        }

        if (isset($this->request->post['filter_billing_type'])) 
		{
            		$filter_billing_type = $this->request->post['filter_billing_type'];
        }
              else if (isset($this->request->get['filter_billing_type'])) 
	{
            		$filter_billing_type = $this->request->get['filter_billing_type'];
        	} 
	else 
	{
            		$filter_billing_type= 'ALL';
        	}

        if (isset($this->request->post['filter_store_2'])) {
            $filter_store = $this->request->post['filter_store_2'];
        }
               else if (isset($this->request->get['filter_store_2'])) {
            $filter_store = $this->request->get['filter_store_2'];
        } else {
            $filter_store = 0;
        }
        if (isset($this->request->post['filter_unit_2'])) {
            $filter_unit = $this->request->post['filter_unit_2'];
                ...
                else if (isset($this->request->get['filter_unit_2'])) {
            $filter_unit = $this->request->get['filter_unit_2'];
        }else {
            $filter_unit = 0;
        }
		$this->load->model('report/reconciliation');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	   ...
	   else
	   {
		...
		'orderproducts'=>$orderproducts
			...
			////////////////for new////////////////
			else if($filter_billing_type=="NEW")
			{
			if(strlen($inv_no)<10)
			{
                    			$data['orders'][] = array(
...
public function get_store_unit_id()
{
$store_id=$this->request->get['store_id'];
if($store_id=="20") 
{
