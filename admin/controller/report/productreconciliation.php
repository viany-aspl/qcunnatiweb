...
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel2007');
	...
	public function reconcliation() {
        ...
        {
			$this->load->model('report/productreconciliation');
            //$t2 = $this->model_report_productreconciliation->getTotalproductreconciliation($filter_data);
            //$order_total2=$t2["total"];
            //$data["total_sold"]=$t2["total_quantity"];
			$results = $this->model_report_productreconciliation->getproductreconciliation($filter_data);
        }
		//print_r($results); 
        $data['orders2'] = array();
		...
		{ 
            $data['orders'][] = array(
                    ...
                    'po_received_qnty'  => $result["po_received_qnty"],
                    'stock_received_qnty'   => $result['stock_received_qnty'],
					'stock_transit' => $result['stock_transit'],
					'po_transit' => $result['po_transit'],
					'sale_qnty' => $result['saleqnty']
                    );
		}
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('report/productreconciliation/reconcliation', 'token=' . $this->session->data['token'] . $url . '&page={page}&tab=1', 'SSL');
		...
		$data['breadcrumbs'] = array();
		$this->load->language('common/information');
		$data['tool_tip']=$this->language->get('report/productreconciliation/reconcliation');
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Product Reconciliation',
			'href' => $this->url->link('report/productreconciliation/reconcliation', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
        ...
        {
            $data["text_no_results"]=" No result found for this store ";
        }
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
        $this->response->setOutput($this->load->view('report/productallreport.tpl', $data)); 
