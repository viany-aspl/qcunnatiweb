<?php
class ControllerReportEleadger extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/eleadger', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		
                	$this->load->model('report/estorelazer');
		...
		$data['store_address']=$this->model_report_estorelazer->getStoreaddress($filter_data);
		$data['store_type']=$this->model_report_estorelazer->getStoreType($filter_data);
		$data['store_type_id']=$this->model_report_estorelazer->getStoreType_id($filter_data);
		$data['store_incharge']=$this->model_report_estorelazer->getStoreInCharge($filter_data);
		$data['store_gstn']=$this->model_report_estorelazer->getStoreGstn($filter_data);
		$data['store_type_id']="3";

		if($filter_stores_id!="")
		{ //echo "here";
			if(($data['store_type_id']=="1") || ($data['store_type_id']=="2"))
			{ //echo "here";
		 		$ord_total = $this->model_report_estorelazer->getTotalTransaction_Own($filter_data);
				$product_total=$ord_total['total'];
				$results = $this->model_report_estorelazer->getTransaction_Own($filter_data);
			}
			else
			{ 
                              	$ord_total = $this->model_report_estorelazer->getTotalStoreledger($filter_data);
				$product_total=$ord_total['total'];
				$results = $this->model_report_estorelazer->getStoreledger($filter_data);
			}
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				'Date'      => $result['datee'],
				'amt_paid'      => $result['amt_paid'],				
				'goods_worth'   => $result['goods_worth'],
		...
		$pagination->url = $this->url->link('report/eleadger', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		if(($data['store_type_id']=="1") || ($data['store_type_id']=="2"))
		{
		  $this->response->setOutput($this->load->view('report/estore_lazer_own.tpl', $data));
		}
		else
		{
                    $this->response->setOutput($this->load->view('report/estore_lazer_franchise.tpl', $data));
		}
	}
