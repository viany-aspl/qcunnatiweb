...
public function salescount() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/product_storewisesales/salescount', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$t1=$this->model_report_product_storewisesales->getTotalsalescount($filter_data);
		$product_total = $t1["total"];
		$data["total_amount_all"]= $t1["total_amount"];
		$total_amount=0;
		$results = $this->model_report_product_storewisesales->getSalescount($filter_data);
		...
		foreach ($results as $result) { //$total_amount=$total_amount+$result['total'];
			$data['products'][] = array(
				...
				'qnty_of_tagged_Subsidy'      => $result['qnty_of_tagged_Subsidy']
			);
		}
		...
		$data['tool_tip']=$this->language->get('report/product_storewisesales/salescount');
		...
		$pagination->url = $this->url->link('report/product_storewisesales/salescount', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/product_store_wise_sales_count.tpl', $data));
	}
...
public function download_excel_salescount(){
    ...
    {         $col = 0;
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $row, $data['qnty_of_tagged_Subsidy']);    
