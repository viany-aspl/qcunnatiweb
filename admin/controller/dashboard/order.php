...
class ControllerDashboardOrder extends Controller {
	public function index() {
		...
		$today = 0;//$this->model_sale_order->getTotalOrders(array('filter_date_added' => date('Y-m-d', strtotime('-1 day'))));

		$yesterday =0;// $this->model_sale_order->getTotalOrders(array('filter_date_added' => date('Y-m-d', strtotime('-2 day'))));
		...
		$filter_data=array('filter_date_added'=>$this->request->get['start_date'],'filter_date_modified'=>$this->request->get['end_date']);
