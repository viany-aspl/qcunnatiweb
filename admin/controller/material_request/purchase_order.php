...
error_reporting(0);
class ControllerMaterialRequestPurchaseOrder extends Controller{
	public function index()
	{
		//set the title of the page
			...
			'href' => $this->url->link('material_request/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('material_request/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('material_request/purchase_order/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['filter'] = $this->url->link('material_request/purchase_order/filter', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['order_list'] = $this->model_materialrequest_purchase_order->getList($start,$limit);
                
		/*getting the list of the orders*/
		
		//getting total orders
		
		$total_orders = $this->model_materialrequest_purchase_order->getTotalOrders();
		
		//getting total orders
		$data['view'] = $this->url->link('material_request/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		$data['receive'] = $this->url->link('material_request/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
		//getting pages
		...
		$pagination->url = $this->url->link('material_request/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('material_request/purchase_order_list.tpl', $data));
	}
	...
	public function add()
	{
		...
		$url = '';
			...
			$data['action'] = $this->url->link('material_request/purchase_order/insert_purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('material_request/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_home'),
			'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], true)
		);
		...
		/*getting the product options from database*/
		//$this->load->model('materialrequest/supplier');
		//$data['suppliers'] = $this->model_materialrequest_supplier->get_total_suppliers();
...
//stores
            ...
            if(isset($user['user_group_id']) && $user['user_group_id']== $pos_user_group_id )
	{
		...
		$this->response->setOutput($this->load->view('materialrequest/purchase_order_form.tpl', $data));
	}
	...
	public function delete()
	{
		...
		$this->load->model('materialrequest/purchase_order');
		$deleted = $this->model_materialrequest_purchase_order->delete($ids);
		if($deleted)
		{
			$_SESSION['delete_success_message'] = "The order successfully deleted";
			$this->response->redirect($this->url->link('materialrequest/purchase_order', 'token=' . $this->session->data['token'] . $url, true));
		}
		else
		{
	...
	public function insert_purchase_order()
	{
		...
		if((in_array("--products--",$data['products'])) || (in_array("--stores--",$data['storess'])) || (in_array("--Product Options--",$data['options'])) || (in_array("--Option Values--",$data['option_values'])) || (count($data['quantity']) != count(array_filter($data['quantity']))) || (count($data['options']) != count(array_filter($data['options']))) || (count($data['products']) != count(array_filter($data['products']))) || (count($data['option_values']) != count(array_filter($data['option_values']))))
		{
			...
			$data['option_values'] = $option_values;
			$url = '';
			$data['action'] = $this->url->link('materialrequest/purchase_order/insert_purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('materialrequest/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			//$this->load->model('materialrequest/supplier');
			//$data['suppliers'] = $this->model_materialrequest_supplier->get_total_suppliers();
		...
		else
		{
			...
			$this->load->model('materialrequest/purchase_order');
			$order_id = $this->model_materialrequest_purchase_order->insert_purchase_order($data);
			...
			if(isset($this->request->post['mail_bit']))
			{
				...
				$data['order_information'] = $this->model_materialrequest_purchase_order->view_order_details($order_id);
		
				
				$html = $this->load->view('materialrequest/mail_purchase_order.tpl',$data);
	...
	public function view_order_details()
	{
		...
		$data['order_information'] = $this->model_materialrequest_purchase_order->view_order_details($order_id);
		$data['cancel'] = $this->url->link('material_request/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['pdf_export'] = $this->url->link('material_request/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		if(isset($_GET['export']))
		{
			...
			$html = $this->load->view('material_request/print_order.tpl',$data);
		...
		{
			$this->response->setOutput($this->load->view('material_request/view_order.tpl',$data));
		}
	}
...
$log->write("filter call 1");
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('materialrequest/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		if(count(array_filter($post,'strlen')) != 0)
		{
			...
			$data['order_list'] = $this->model_materialrequest_purchase_order->filter($filter,$start,$limit);
			if(!$data['order_list'])
			{
			...
			else
			{
				...
				$total_orders = $this->model_materialrequest_purchase_order->filterCount($filter);
			}
		}
		else
		{
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('material_request/purchase_order/filter', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	{
		$this->document->setTitle("Order Dispatch");
		$order_id = $this->request->get['order_id'];
		...
		$data['action'] = $this->url->link('material_request/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link('material_request/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		//$this->load->model('materialrequest/supplier');
		//$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
		$this->response->setOutput($this->load->view('material_request/receive_order.tpl',$data));
...
$log->write("before check");
		if((count($received_quantities) != count(array_filter($received_quantities))) || (count($prices) != count(array_filter($prices))) || $order_receive_date == '')
		{
			...
			$data['breadcrumbs'][] = array(
			'text' => "Purchase Request",
			'href' => $this->url->link('materialrequest/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
			);
			...
			//$this->load->model('materialrequest/supplier');
			//$data['suppliers'] = $this->model_materialrequest_supplier->get_total_suppliers();
			//$this->response->redirect($this->url->link('materialrequest/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true));
			$this->response->setOutput($this->load->view('material_request/receive_order.tpl',$data));
		}
		else
		{
$log->write("after check");
			...
			$this->load->model('materialrequest/purchase_order');
			$inserted = $this->model_materialrequest_purchase_order->insert_receive_order($received_order_info,$order_id);
			if($inserted)
			{
				$_SESSION['receive_success_message'] = 'Order received Successfully!!';
				$this->response->redirect($this->url->link('material_request/purchase_order', 'token=' . $this->session->data['token'] . $url, true));
			}
			else
			{
				$_SESSION['something_wrong_message'] = 'Sorry!! something went wrong, try again';
				$this->response->redirect($this->url->link('material_request/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true));
			}
    ...
    public function dispatchDetail() {
        $log = new Log('Material Request Dispatch Detail ' . date('Y-m-d') . '.log');
        ...
        if ($invoice_no !='') {
            ...
            $data['zi_name'] = $this->request->post['zi_name'];
            $log->write($data);
            //$data['summaryorders']=$summaryresults=$this->model_tagpos_fmdelivery->getRecords2($filter_data);
            $html = $this->load->view('material_request/dispatch_info.tpl', $data, true);
            $msg = array('status' => 'success', 'responce' => $html);
        } else {
            $html = '<div class="form-control">Order not found...</div>';
    ...
    public function process_order(){
       ...
       $driver_mobile_number = $this->request->post['mobile'];
       $vehicle_number = $this->request->post['vehicle_number'];
//       $setcollum= "order_status_id ='".$order_status."'";
       
       if ($order_id !='') {
            
            
            $setcollum= "vehicle_type = '".$vehicle_type."' , driver_name ='".$driver_name."' ,  driver_mobile = '".$driver_mobile_number."' , vehicle_number = '".$vehicle_number."' , order_status_id = '3' ";
            $dispatch_order = $this->model_materialrequest_purchase_order->change_order_status( $order_id, $setcollum);
            //$html = $this->load->view('material_request/dispatch_info.tpl', $data, true);
            $msg = array('status' => 'success', 'responce' => $dispatch_order);
        } else {
