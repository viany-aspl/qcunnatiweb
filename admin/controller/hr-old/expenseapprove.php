...
class ControllerHrExpenseapprove extends Controller {
	private $error = array();
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('hr/expenseapprove', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$filter_data = array(
			...
			'limit'                  => $this->config->get('config_limit_admin')
                ...
                $order_total = $this->model_hr_hr->getTotalbill($filter_data);
	 
		$results = $this->model_hr_hr->getBills($filter_data);
 	  $mcrypt=new MCrypt();
		...
		$pagination->url = $this->url->link('hr/expenseapprove', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('hr/bill_list_for_approve.tpl', $data));
