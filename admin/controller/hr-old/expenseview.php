...
class ControllerHrExpenseview extends Controller {
	private $error = array();
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('hr/expenseview', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
 	  ...
 	  $mcrypt=new MCrypt();
		...
		$pagination->url = $this->url->link('hr/expenseview', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('hr/bill_list.tpl', $data));
