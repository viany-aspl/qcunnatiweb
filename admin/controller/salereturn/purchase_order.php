...
error_reporting(0);
class ControllerSalereturnPurchaseOrder extends Controller{
  public function check_ware_house_quantity()
        {
            
            $total_product=count($this->request->post['product_id']);
            $ware_house=$this->request->post['store_to'];
            ...
            for($a=0;$a<$total_product;$a++)
            {
               ...
               $this->load->model('salereturn/purchase_order');
               $data_qnty=$this->model_salereturn_purchase_order->check_ware_house_quantity($ware_house,$product_id,$p_qnty);
               $data_price=$this->model_salereturn_purchase_order->check_ware_house_price($ware_house,$product_id,$p_price);
               if($data_qnty=="0")
               {
            ...
            exit;
            //$data_credit=$this->model_salereturn_purchase_order->check_ship_to_credit($ship_to,$grand_total);
            //if($data_credit=="0")
            //{
   ...
   public function index()
	{
			$this->document->setTitle("Sale Return");
		...
		if (isset($this->request->get['filter_store'])) {
			...
			$data['breadcrumbs'][] = array(
			'text' => "Sale Return",
			'href' => $this->url->link('salereturn/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('salereturn/purchase_order/invoice', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['order_list'] = $this->model_salereturn_purchase_order->getList($filter_data);
		
		$total_orders = $this->model_salereturn_purchase_order->getTotalOrders($filter_data);
		
		$data['view'] = $this->url->link('salereturn/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		//$data['receive'] = $this->url->link('partner/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
                $data['invoice'] = $this->url->link('salereturn/purchase_order/download_invoice', 'token=' . $this->session->data['token'] . $url, true);
		...
		$pagination->url = $this->url->link('salereturn/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('salereturn/salereturn_list.tpl', $data));
	}
	...
	public function autocomplete() {
        ...
        public function download_invoice()
        {
             ...
             $this->load->model('salereturn/purchase_order');
             $data['order_information'] = $this->model_salereturn_purchase_order->view_order_details_for_created_invoice($order_id); 
             //print_r($data['order_information']['products']);
             $data['store_to_data']=$this->model_salereturn_purchase_order->get_to_store_data($data['order_information']['order_info']['store_from']);
             $data['created_po']=$data['order_information']['order_info']['po_invoice_n'];
             $data['invoice_id']=$order_id;
             $this->response->setOutput($this->load->view('salereturn/order_invoice_print.tpl',$data));
             
             
             
             $html=$this->load->view('salereturn/order_invoice_print.tpl',$data);
             require_once(DIR_SYSTEM .'/library/mpdf/mpdf.php');
	...
	public function invoice()
	{
		...
		$data['column_left'] = $this->load->controller('common/column_left');
                ...
                if($this->request->server['REQUEST_METHOD'] == 'POST')
                {
                    ...
                    $store=$this->request->post['store_to'];
                    $ship_to=$this->request->post['ship_to'];
                    $grand_total=$this->request->post['grand_total'];
                    ...
                    for($a=0;$a<$total_product;$a++)
                    {
                        ...
                        $this->load->model('partner/purchase_order');
                        $data_qnty=$this->model_partner_purchase_order->check_ware_house_quantity($data['store_to'],$product_id,$p_qnty);
                        $data_price=$this->model_partner_purchase_order->check_ware_house_price($data['store_to'],$product_id,$p_price);
                        if($data_qnty=="0")
                        {
                            $this->session->data['error_warning']='There is not sufficent quantity of '.$product_name.' at ware house';
                            $this->response->redirect($this->url->link('salereturn/purchase_order/invoice', 'token=' . $this->session->data['token'] . $url, true)); 
                        }
                        if($data_price=="0")
                        {
                    ...
                    $data_credit=$this->model_partner_purchase_order->check_ship_to_credit($store,$grand_total);
                    if($data_credit=="0")
                    {
                    ...
                    $submit_d=$this->model_salereturn_purchase_order->submit_po_invoice($this->request->post);
                    $invoice_number=$submit_d['invoice_no'];
                    $invoice_id=$submit_d['invoice_id'];
                    //print_r($submit_d);exit;
                    $this->session->data['success']='Sale return Created Successfully ';
...
$trans->storewalletcredit($store,$grand_total,'');
$trans->storecurrentcreditcredit($store,$grand_total,'');
$trans->addstoretrans($grand_total,$store,$updated_by,'CR',$submit_d['invoice_id'],"Sale Return",$grand_total,'');
                    ...
                    $this->response->redirect($this->url->link('salereturn/purchase_order', 'token=' . $this->session->data['token'] . $url, true));
                }
		...
		$data['token']=$this->session->data['token'];
                $data['cancel'] = $this->url->link('salereturn/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
                
                $created_po=$this->model_salereturn_purchase_order->check_po_invoice($order_id);
                $created_po="";
                if($created_po>0)
                {
                   $data['order_information'] = $this->model_salereturn_purchase_order->view_order_details_for_created_invoice($order_id); 
                ...
                {
                   $data['order_information'] = $this->model_salereturn_purchase_order->view_order_details_invoice($order_id); 
...
// print_r($data['order_information']);
		...
		$this->response->setOutput($this->load->view('salereturn/order_invoice.tpl',$data));
...
<img src="https://unnati.world/shop/image/letterhead_topline.png" style="height: 10px; width: 120% !important;" /> 
	...
	public function b2b()
	{
		...
		$data['order_list'] = $this->model_salereturn_purchase_order->getList_b2b($filter_data);
		
		$total_orders = $this->model_salereturn_purchase_order->getTotalOrders_b2b($filter_data);
		
		$data['view'] = $this->url->link('salereturn/purchase_order/view_order_details_b2b', 'token=' . $this->session->data['token'] . $url, true);
		//$data['receive'] = $this->url->link('partner/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
                $data['invoice'] = $this->url->link('salereturn/purchase_order/download_invoice_b2b', 'token=' . $this->session->data['token'] . $url, true);
		if (isset($this->session->data['success'])) {
			$data['success'] = $this->session->data['success'];
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('salereturn/purchase_order/b2b', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['filter_date_start']=$filter_date_start ;
                ...
                $data['token']=$this->request->get['token'];
		$data['my_custom_text'] = "This is purchase order page.";
		$this->response->setOutput($this->load->view('salereturn/invoice_list_b2b.tpl', $data));
	}
...
<img src="https://unnati.world/shop/image/letterhead_topline.png" style="height: 10px; width: 120% !important;" /> 
		...
		public function download_excel()
	{
		...
		$this->load->model('salereturn/purchase_order');
		$results=$this->model_salereturn_purchase_order->download_excel($filter_data);
		//print_r($results);
		//exit;

		$file_name="Sale_return_data_".date('dMy').'.xls';
...
foreach($results as $data)
{
		...
		$total_tax=($data['p_price']*$data['p_qnty']*18)/100;
 		...
 		else if($data['p_tax_type']=="GST@28%")
	      {
		echo '<td>'.number_format((float)28, 2, '.', '').'</td>';
		$total_tax=($data['p_price']*$data['p_qnty']*28)/100;
	      }
