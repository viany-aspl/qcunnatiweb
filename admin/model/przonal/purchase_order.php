...
class ModelPrzonalPurchaseOrder extends Model {
...
(select store_id from oc_setting  where oc_setting.`key`='config_storetype' and oc_setting.value in ('2','4') )";
    ...
    public function getList($data) {
        $sql = "SELECT
			...
			,oc_pr_order.name as product
			,oc_pr_order.quantity
                        FROM
			oc_pr_order
			LEFT JOIN
oc_pr_order ON (oc_pr_order.id= oc_pr_order.order_id)
...
// if (!empty($data['filter_status'])) { 
    ...
    public function insertdocument_upload_type($data) {
        ...
        $sql = "update oc_pr_order  SET po_number='" . $data['po_number'] . "',pr_varification_image='" . $data['pr_varification_image'] . "',image_upload_date=NOW()";
        $log->write($sql);
