...
class Modelleadorderleads extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id,o.store_name,o.date_potential, CONCAT(o.firstname, ' ', o.lastname) AS customer,o.telephone,ocu.firstname as firstnamea,ocu.lastname as lastnamea, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.shipping_code, o.total,(SELECT total from oc_order where `comment`=o.order_id) as 'aTotal', o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order_leads` o left join oc_user as ocu on ocu.user_id=o.user_id  ";
		...
		if (!empty($data['filter_total'])) {
			$sql .= " AND o.total = '" . (float)$data['filter_total'] . "'";
                            ...
                            if (!empty($data['filter_addeddby'])) {
			$sql .= " AND o.user_id in ( SELECT user_id FROM `oc_user` where `user_group_id`='".$this->db->escape($data['filter_addeddby'])."'  )";
		}
...
//echo $sql;
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_total'])) {
 		 ...
 		 if (!empty($data['filter_addeddby'])) {
			$sql .= " AND user_id in ( SELECT user_id FROM `oc_user` where `user_group_id`='".$this->db->escape($data['filter_addeddby'])."'  )";
		}
	...
	public function getbill_to_requisition($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "requisition_to_bill WHERE bill_id = '" . $order_id . "'");
		return $query->row['requisition_id'];
	}
	public function getrequisition_to_bil($order_id) 
	{
        $sql="SELECT order_id as bill_id FROM " . DB_PREFIX . "order WHERE comment = '" . $order_id . "' and order_status_id='5'  ";
		$sql .= " AND (date_added) >= CAST('" . ((date('Y')-1).'-12-31') . "' as DATETIME) ";
		...
		if($query->num_rows)
		{
            $log->write($query->row['bill_id']);
			return $query->row['bill_id'];
		}
		$sql2="SELECT bill_id FROM " . DB_PREFIX . "requisition_to_bill WHERE requisition_id = '" . $order_id . "'";
		$sql2 .= " AND (create_time) >= CAST('" . ((date('Y')-1).'-1-31') . "' as DATETIME) ";
	...
	public function getrequisition_to_bil_indent($order_id) 
	{
        $log=new Log("reconciliation-to-bill-".date('Y-m-d').".log");
		$sql2="SELECT bill_id FROM " . DB_PREFIX . "requisition_to_bill WHERE requisition_id = '" . $order_id . "'";
		$sql2 .= " AND (create_time) >= CAST('" . ((date('Y')-1).'-12-31') . "' as DATETIME) ";
