...
//if($data["filter_date_start"]==$data["filter_date_end"])
			{
                ...
                $sql="select 
				sum(case when o.payment_method='Cash' then p.quantity else 0 end)as qnty_of_cash, 
				sum(case when o.payment_method='Tagged' then p.quantity
					WHEN o.payment_method = 'Tagged Subsidy' and p.SubsidyAmount = 0 THEN p.quantity
				else 0 end)qnty_of_tagged,
				sum(case when o.payment_method='Tagged Cash' then p.quantity else 0 end)as qnty_of_tagged_cash,
				sum(case when o.payment_method='Subsidy' then p.quantity else 0 end)as qnty_of_Subsidy,
				sum(case when o.payment_method='Tagged Subsidy' and p.SubsidyAmount>0 then p.quantity else 0 end)as qnty_of_tagged_Subsidy,
				o.store_name,p.product_id, p.name from oc_order_product p left JOIN oc_order o on o.order_id=p.order_id ";
...
//if($data["filter_date_start"]==$data["filter_date_end"])
			{
                ...
                $sql="select sum(case when o.payment_method='Cash' then p.quantity else 0 end)as qnty_of_cash, 
				sum(case when o.payment_method='Tagged' then p.quantity
					WHEN o.payment_method = 'Tagged Subsidy' and p.SubsidyAmount = 0 THEN p.quantity
				else 0 end)qnty_of_tagged,
				sum(case when o.payment_method='Tagged Cash' then p.quantity else 0 end)as qnty_of_tagged_cash,
				sum(case when o.payment_method='Subsidy' then p.quantity else 0 end)as qnty_of_Subsidy,
				sum(case when o.payment_method='Tagged Subsidy' and p.SubsidyAmount>0 then p.quantity else 0 end)as qnty_of_tagged_Subsidy,
				o.store_name,p.product_id, p.name from oc_order_product p
