...
class ModelReportDsclInputSales extends Model {
	...
	public function getcash($store_id,$date,$unit_id)       
        {
            $sql="SELECT sum(cash) as 'Cash' FROM `oc_order` o where payment_method in ('Cash','Tagged Cash','Cash Subsidy','Tagged')
			and o.order_status_id=5 
			and date(date_added)='".$date."' 
			and `store_id`='".$store_id."' ";
			if(($store_id=='20') || ($store_id=='80'))
			{
				$sql.=" and o.unit_id='".$unit_id."' ";
				//echo $sql;	
			} 
        ...
        public function gettagged($store_id,$date,$unit_id)
        {
            $sql="SELECT sum(tagged) as 'Tagged' FROM `oc_order` o where payment_method in ('Tagged','Tagged Cash','Tagged Subsidy','Tagged') 
			and o.order_status_id=5 and date(date_added)='".$date."'  
			and `store_id`='".$store_id."'"; 
			if(($store_id=='20') || ($store_id=='80'))
			{
				$sql.=" and o.unit_id='".$unit_id."' ";
        ...
        public function getcashtilldate($store_id,$filter_date='',$unit_id,$data=array())       
        {
            $sql="SELECT sum(cash) as 'Cash' FROM `oc_order` o where o.payment_method in ('Cash','Tagged Cash','Cash Subsidy','Tagged')
			and o.order_status_id=5 
			and o.store_id='".$store_id."' ";
			
			//and date(date_added)>='".date('Y').'-01-01'."'  ";
			if (!empty($data['filter_year_start'])) 
			{
				$sql .= " AND (o.date_added) >= CAST('" . $this->db->escape($data['filter_year_start']) . " 00:00:00' as datetime)";
			}  
			if (!empty($data['filter_year_end'])) 
			{
				$data['filter_year_end']=date('Y-m-d',strtotime($data['filter_year_end'] . "+1 days"));
				$sql .= " AND (o.date_added) <= CAST('" . $this->db->escape($data['filter_year_end']) . " 00:00:00' as datetime)";
			} 
			if(($store_id=='20') || ($store_id=='80'))
			{
				$sql.=" and o.unit_id='".$unit_id."' ";
            ...
            {
				$filter_date=date('Y-m-d',strtotime($filter_date . "+1 days"));
				$sql .= " AND (o.date_added) <= CAST('" . $this->db->escape($filter_date) . " 00:00:00' as datetime)";
...
{	
        ...
        public function gettaggedtilldate($store_id,$filter_date='',$unit_id,$data=array())
        {
            $sql="SELECT sum(tagged) as 'Tagged' FROM `oc_order` o where o.payment_method in ('Tagged','Tagged Cash','Tagged Subsidy','Tagged')  
			and o.order_status_id=5 
			and o.store_id='".$store_id."' ";
			//and date(date_added)>='".date('Y').'-01-01'."' "; 
			if (!empty($data['filter_year_start'])) 
	...
	//echo $sql;
	 ...
	 return $query->rows;
        ...
        {
         $sql="select oc_store.name,oc_store.store_id,oc_unit.unit_name, oc_unit.unit_id as unit_id
		 from oc_store 
		 left join oc_setting on oc_store.store_id=oc_setting.store_id 
		 left join oc_store_to_unit on oc_store.store_id=oc_store_to_unit.store_id 
		 left join oc_unit on oc_store_to_unit.unit_id=oc_unit.unit_id 
		 where oc_setting.key='config_company' and oc_setting.value='1' 
		 and oc_store.store_id not in (14,52,47,46,55,53) ";
		 if(!empty($data['filter_unit']))
		 {
			 $sql.=" and oc_store_to_unit.unit_id='".$data['filter_unit']."' ";
		 }
		 $sql.=" group by oc_store_to_unit.unit_id,oc_store_to_unit.store_id
		 order by oc_unit.unit_name,oc_store.name asc  ";   
	 ...
	 return $query->rows;
        ...
        public function getstoresTotal($data)
        {
         $sql="select count(*) as total from ( select oc_store.store_id from oc_store 
		 left join oc_setting on oc_store.store_id=oc_setting.store_id 
		 left join oc_store_to_unit on oc_store.store_id=oc_store_to_unit.store_id 
		 left join oc_unit on oc_store_to_unit.unit_id=oc_unit.unit_id 
		 where oc_setting.key='config_company' and oc_setting.value='1' 
