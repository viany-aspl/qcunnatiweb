...
oc_po_product ON (oc_po_order.id= oc_po_product.order_id)
    ...
    public function insert_purchase_order($data = array()) {
        ...
        if ($data['supplier_id'] != "--Supplier--") {
            $this->db->query('INSERT INTO oc_po_order (order_date,user_id,pre_supplier_bit) VALUES("' . NOW() . '",' . $this->session->data['user_id'] . ',1)');
            $order_id = $this->db->getLastId();
        } else {
            $this->db->query('INSERT INTO oc_po_order (order_date,user_id) VALUES("' . NOW() . '",' . $this->session->data['user_id'] . ')');
            $order_id = $this->db->getLastId();
        }
