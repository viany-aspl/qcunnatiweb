...
class ModelMaterialrequestPurchaseOrder extends Model {

    public function getListStore($uid, $start, $limit, $order_id) {

        $log = new Log("inv-material-request-store-modal-all-" . date('Y-m-d') . ".log");
        $sql = "SELECT
            oc_po_order.*
            ,'MR' as 'receivetype'
    ...
    public function get_driver_otp($order_id) {
        ...
        $log->write($order_id);
        $sql = "update oc_po_order set driver_otp='1234'  WHERE  oc_po_order.id='" . $order_id . "' LIMIT  1";
        $log->write($sql);
        $query = $this->db->query($sql);
        $log->write($query->row['driver_otp']);
        return $query->row['driver_otp'];
    ...
    public function insert_purchase_order($data = array()) {
        $log = new Log('mr-po-order-' . date('Y-m-d') . ".log");
        ...
        for ($i = 0; $i < count($data['products']); $i++) {
            ...
            if ($data['supplier_id'] != "--Supplier--") {
                $sql1 = 'INSERT INTO oc_po_order (order_date,user_id,pre_supplier_bit,store_id,store_type,potential_date) VALUES("' . NOW() . '",' . $this->session->data['user_id'] . ',1,' . $data['stores'][$i][0] . ',"' . $data['store_type'] . '","' . $data['potentialdate'] . '")';
                $log->write($sql1);
                $this->db->query($sql1);
                $order_id = $this->db->getLastId();
            } else {
                $sql2 = 'INSERT INTO oc_po_order (order_date,user_id,store_id,store_type,potential_date) VALUES("' . NOW() . '",' . $this->session->data['user_id'] . ',' . $data['stores'][$i][0] . ',"' . $data['store_type'] . '","' . $data['potentialdate'] . '")';
                $log->write($sql2);
                $this->db->query($sql2);
                $order_id = $this->db->getLastId();
            }
            ...
            if ($data['supplier_id'] != "--Supplier--") {
                ...
                $query = $this->db->query("INSERT INTO oc_po_receive_details (quantity,product_id,supplier_id,order_id,store_id) VALUES(" . $data['quantity'][$i] . "," . $data['products'][$i][0] . ",-1," . $order_id . ",'" . $data['stores'][$i][0] . "')");
            }
        }
    ...
    public function getListRecStore($sr_dat) {

        $log = new Log("inv-material-request-modal-" . date('Y-m-d') . ".log");
        $sdata = "";
        if (in_array('status', $sr_dat) && $sr_dat['status'] != '') {
            $sdata .= " AND oc_po_order.order_status_id='" . $sr_dat['status'] . "' ";
     } 
     //else {
//            $sdata .= " AND oc_po_order.order_status_id='0' ";
//        }
        if (in_array('uid', $sr_dat) && $sr_dat['uid'] != '') {
            $sdata .= " and oc_po_order.user_id='" . $sr_dat['uid'] . "' ";
        }
        if (in_array('order_id', $sr_dat) && $sr_dat['order_id'] != '') {
            $sdata .= " and oc_po_order.id='" . $sr_dat['order_id'] . "' ";
        }
        if (in_array('store_id', $sr_dat) && $sr_dat['store_id'] != '') {
            $sdata .= " and oc_po_order.store_id='" . $sr_dat['store_id'] . "' ";
        }
        $log->write($sr_dat);
        $log->write($sdata);
        $sql = "SELECT
			...
			,'MR' as 'receivetype'
                        ...
                        ,oc_po_receive_details.quantity
                        ,CONCAT_WS(' ',oc_user.firstname,oc_user.lastname) as username
			FROM oc_po_order
                        INNER JOIN oc_po_receive_details
				ON (oc_po_order.id = oc_po_receive_details.order_id)
			...
			INNER JOIN " . DB_PREFIX . "user 
				ON (oc_po_order.user_id = " . DB_PREFIX . "user.user_id) WHERE oc_po_order.order_sup_send = '0000-00-00' AND oc_po_order.delete_bit = 1 AND pre_supplier_bit='0' " . $sdata;

        $sql .= " GROUP BY oc_po_order.id ORDER BY oc_po_order.id DESC";
//                LEFT JOIN oc_po_supplier 
//				ON (oc_po_receive_details.supplier_id = oc_po_supplier.id)

        if (in_array('limit', $sr_dat) && $sr_dat['limit'] != '' || in_array('start', $sr_dat) && $sr_dat['start'] != '') {
            if (!isset($sr_dat['start']) || $sr_dat['start'] < 0) {
                $sr_dat['start'] = 0;
            }

            if (!isset($sr_dat['limit']) || $sr_dat['limit'] < 1) {
                $sr_dat['limit'] = 20;
            }

            $sql .= " LIMIT " . $sr_dat['start'] . "," . $sr_dat['limit'];
        }
    ...
    public function getList($start, $limit) {
        $log = new Log("material-request-modal-" . date('Y-m-d') . ".log");
        $sql="SELECT
			oc_po_order.*
			...
			LEFT JOIN oc_po_supplier 
				...
				ON (oc_po_receive_details.product_id = oc_product.product_id)
			INNER JOIN " . DB_PREFIX . "user 
...
$log->write(vinay11-11);
    ...
    public function get_store_data($store_id, $select = '*') {
        //$log=new Log("getstorename.log");
        $sql = "SELECT " . $select . " FROM oc_store where store_id='" . $store_id . "'";
        //$log->write($sql);
        $query = $this->db->query($sql);
        //$log->write($query);
        return $query->rows;
    ...
    public function change_order_status($id, $setcollum = 'order_status_id=\'1\'') {

        $sql = "UPDATE oc_po_order SET " . $setcollum . " WHERE id = '" . $id . "'";

        $query = $this->db->query($sql);
        if ($query == 1) {
            $message = "Order " . $id . " has been updated sucessfully";
        } else {
