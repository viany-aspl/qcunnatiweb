<?php
class ModelSaleOrderleads extends Model {
	...
	public function getOrderProducts($order_id) {
                $pp_sql="SELECT *,(SELECT subsidy FROM " . DB_PREFIX . "product_subsidy pds WHERE pds.product_id = oc_order_product_leads.product_id AND pds.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pds.quantity = '1'  ORDER BY pds.priority ASC  LIMIT 1) AS subsidy FROM " . DB_PREFIX . "order_product_leads WHERE order_id = '" . (int)$order_id . "'";
		$query = $this->db->query($pp_sql); 
...
public function getTop_5_Orders($data=array()) {
                            $start_date=date('Y-m')."-01";
                             $end_date=date('Y-m-d');
		$query = $this->db->query(" 
select total,store_name from (
select (Cash+Tagged+Subsidy)as total,store_name from (
select * from (
select Cash,Tagged,Subsidy,a.store_id,a.store_name,st.creditlimit,
st.currentcredit,cash_order,tagged_order,Subsidy_order from
 ( select sum(Cash) as 'Cash' ,sum(Tagged) as 'Tagged',sum(Subsidy)as Subsidy,
 store_id,store_name,sum(cash_order)as cash_order,sum(tagged_order)as tagged_order,
 sum(Subsidy_order)as Subsidy_order from
 ( SELECT sum(total) as 'Cash','0' as 'Tagged','0' as 'Subsidy',store_id,
 store_name,date_added,count(order_id)as cash_order,'0' as tagged_order,
 '0' as 'Subsidy_order' FROM `oc_order` o where payment_method='Cash'
 AND DATE(o.date_added) >= '". $start_date."' AND DATE(o.date_added) <= '".$end_date."'
 GROUP BY DATE(o.date_added),o.store_id
 
 UNION all
 
 SELECT '0' as 'Cash',sum(total) as 'Tagged','0' as 'Subsidy',store_id,store_name,
 date_added,'0' as cash_order, count(order_id)as tagged_order,'0' as 'Subsidy_order'
 FROM `oc_order` o where (payment_method='Tagged' or payment_method='Tagged Cash')
 AND DATE(o.date_added) >= '". $start_date."' AND DATE(o.date_added) <= '".$end_date."'
 GROUP BY DATE(o.date_added),o.store_id
 
 union all
 
 SELECT '0' as 'Cash','0' as 'Tagged',
 sum(total) as 'Subsidy', store_id,store_name,date_added,'0' as cash_order,'0' as tagged_order,
 count(order_id)as 'Subsidy_order' FROM `oc_order` o where
 payment_method='Subsidy' AND DATE(o.date_added) >= '". $start_date."'
 AND DATE(o.date_added) <= '".$end_date."' GROUP BY DATE(o.date_added),o.store_id ) as tt
 GROUP BY tt.store_id) as a left join oc_store as st on st.store_id = a.store_id
 )as aaa
 )as ccc 
 )as dd order by total desc limit 0,5
...
$sql=" 
select * from (
select oc_order_product.product_id as product_id,sum(oc_order_product.quantity)as sales_of_qnty,oc_product.model from oc_order_product join oc_product on oc_product.product_id=oc_order_product.product_id
where date(ORD_DATE) between '".$start_date."' and '".$end_date."'
group by product_id
    ) as aa order by sales_of_qnty desc
 ";
		$query = $this->db->query($sql);
		return $query->rows;
	}
