...
class ModelSaleOrder extends Model 
{
	...
	public function updateDsclDeliverStatus($order_id,$status,$dscl_trans_id)
	{
		$log=new Log("submit_order_to_dscl_pink-".date('Y-m-d').".log");
		$sql="update oc_order set dscl_upload='1',dscl_trans_id='".$dscl_trans_id."',dscl_upload='".$status."' where order_id='".$order_id."' ";
		$log->write($sql);
		$query=$this->db->query($sql);
	}
	public function batch_update($data=array())
	{
		$log=new Log("batch_update-".date('Y-m-d').".log");
		$sql="update oc_req_batch set status='1',delivery_date='".date('Y-m-d')."',otp='".$data['otp']."' where sid='".$data['batch_no']."' ";
		$log->write($sql);
		$query=$this->db->query($sql);
	}
	public function batch_fm_set($data=array())
	{
		$log=new Log("batch_update-".date('Y-m-d').".log");
		
		$sql="update oc_req_batch set otp_fm_code='".$data['fm_id']."' where sid='".$data['batch_no']."' ";
		$log->write($sql);
		$query=$this->db->query($sql);
		
	}
	public function batch_view($data=array())
	{
		$sql=" select * from oc_req_batch where fm_code='".$data['fm_code']."' and date(filter_date)>='".$data['start_date']."' and date(filter_date)<='".$data['end_date']."' and status=0 ";
...
FROM
 ...
 join (SELECT 
        ...
        WHERE
            oc_order_delivery_advance.fmcode = '".$data['fm_code']."'
        ...
        WHERE
            oc_order_delivery.fmcode = '".$data['fm_code']."'
		...
		if ($order_query->num_rows) 
		{
			...
			*/
			...
			return array(
				...
				'grower_id'		=>   $order_query->row['grower_id'],
				'subsidy_form_no'		=>   $order_query->row['subsidy_form_no'],
				'subsidy_coupon'		=>   $order_query->row['subsidy_coupon'],
				'dscl_upload'		=>   $order_query->row['dscl_upload'],
				'dscl_trans_id'		=>   $order_query->row['dscl_trans_id'],
				'subsidy_category'		=>   $order_query->row['subsidy_category'],
			);
		} else {
			//$log->write('in else');
			return;
	...
	public function getSubsidyOrders($data = array())
	{
		...
		$sql="select * from oc_order as o where o.payment_method in ('Subsidy','Tagged Subsidy') and o.order_status_id=5 
		AND (o.subsidy_form_no is null) ";
		...
		{
			if ((empty($data['filter_order_id'])) && (empty($data['grower_id'])))
			{
	...
	public function getOrders($data = array()) 
	{
		...
		$sql = "SELECT o.user_id,o.order_id,o.comment as req_id,o.subsidy_coupon,o.payment_method as payment_method, 
		CONCAT(o.firstname, ' ', o.lastname) AS customer,o.shipping_firstname,
		o.telephone, (SELECT os.name 
		FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id 
		AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, 
		o.shipping_code, o.total,o.tagged,o.cash, o.currency_code, o.currency_value, o.date_added, o.date_modified,
		o.payment_method,o.store_name,o.store_id,CONCAT(o.invoice_prefix,'-',o.invoice_no) as `invoice_no`,
		concat(oc_user.firstname,' ',oc_user.lastname) as op_name
		FROM `" . DB_PREFIX . "order` o
		left join oc_user on o.user_id=oc_user.user_id
		";
		//$log->write($sql);
		...
		{
			if($data['filter_payment']=='Tagged & Tagged Cash & Tagged Subsidy')
			{
				$sql .= " AND o.payment_method in  ('Tagged','Tagged Cash','Tagged Subsidy')";
			}
			else
			{
		...
		if (!empty($data['filter_date_added'])) 
		{
			if(empty($data['filter_fm_id']))
			{
				$sql .= " AND (o.date_added) >= CAST('" . $this->db->escape($data['filter_date_added']) . " 00:00:00' as datetime)";
			}
		}
		...
		if (!empty($data['filter_date_modified'])) 
		{
			if(empty($data['filter_fm_id']))
			{
				$sql .= " AND (o.date_added) <= CAST('" . $this->db->escape($data['filter_date_modified']) . " 00:00:00' as datetime)";
			}
		}
        ...
        if(!empty($data['filter_user_id']))
		{
			if(!empty($data['filter_fm_id']))
			{
				$sql .= " AND o.fmcode = '" . (int)$data['filter_fm_id'] . "'";
			}
			else
			{
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_payment'])) { 
			if($data['filter_payment']=='All-Tagged')
			{ //echo $data['filter_payment'];
		...
		if (!empty($data['filter_date_added'])) {
			$sql .= " AND (date_added) >= CAST('" . $this->db->escape($data['filter_date_added']) . " 00:00:00' as datetime)";
		}
		$data['filter_date_modified']=date('Y-m-d',strtotime($data['filter_date_modified'] . "+1 days"));
		if (!empty($data['filter_date_modified'])) {
			$sql .= " AND (date_added) <= CAST('" . $this->db->escape($data['filter_date_modified']) . " 00:00:00' as datetime)";
		}
...
public function getOrderSubsidy($order_id) {
	...
	public function getOrder_detail_subsidy_product($order_id) 
	{


	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE discount_value>0 and  order_id = '" . (int)$order_id . "'");
	...
	public function getOrder_detail_total($order_id) 
	{
...
public function getTop_5_Products($data=array()) {
 ...
 //echo $sql;
	...
	public function getCashVillageCode($order_id) 
	{
		$query = $this->db->query("SELECT * from oc_bcml_cash_order_trans WHERE order_id = '" . $order_id . "'");
		return $query->row;
	}
    ...
    //company wise
public function getFMOrdersCompanywise($data = array()) {
...
$sql.= "AND od.invoice_no = '".(int)$this->db->escape($data['filter_invoice'])."' ";
}
...
if (isset($data['start']) || isset($data['limit'])) {
        ...
        public function getFMTotalOrdersCompanywise($data = array()) {
...
{
$sql.= "AND od.invoice_no = '".(int)$data['filter_invoice']."' ";
}
