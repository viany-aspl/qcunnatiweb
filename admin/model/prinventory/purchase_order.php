<?php
class ModelPrinventoryPurchaseOrder extends Model {
public function getListStore($uid,$start,$limit,$order_id)
    {
        ...
        $sql="SELECT
            ...
            FROM
            oc_pr_order
            INNER JOIN oc_pr_receive_details
                ON (oc_pr_order.id = oc_pr_receive_details.order_id)
            LEFT JOIN oc_po_supplier
                ON (oc_pr_receive_details.supplier_id = oc_po_supplier.id)
            INNER JOIN ".DB_PREFIX."user
                ON (oc_pr_order.user_id = ".DB_PREFIX."user.user_id) WHERE  oc_pr_order.user_id='".$uid."'";
               
            if($order_id!="")
            {
            $sql.=" and oc_pr_order.id='".$order_id."' ";
            }
	$sql.=" and (oc_pr_order.order_date)>=(select value as reg_date from oc_setting where store_id=oc_pr_order.store_id and `key`='config_registration_date' limit 1) ";
                $sql.=" AND oc_pr_order.delete_bit = 1  GROUP BY oc_pr_order.id ORDER BY oc_pr_order.id DESC LIMIT " . $start . "," . $limit;
                $log->write($sql);
...
public function check_driver_otp($order_id)
	{
		...
		$log->write($order_id);
		$sql="SELECT oc_pr_order.driver_otp as driver_otp FROM oc_pr_order WHERE  oc_pr_order.id='".$order_id."' LIMIT  1";
		$log->write($sql);
		$query = $this->db->query($sql);
        ...
        public function get_driver_otp($order_id)
	{
		$log=new Log("receive-product-".date('Y-m-d').".log");

		$log->write($order_id);
		$sql="update oc_pr_order set driver_otp='1234'  WHERE  oc_pr_order.id='".$order_id."' LIMIT  1";
		$log->write($sql);
		$query = $this->db->query($sql);
		$log->write($query->row['driver_otp']);
		return $query->row['driver_otp'];
...
public function check_quantity($order_id)
{
		...
		$log->write($order_id);
		$sql="SELECT oc_pr_product.quantity as quantity FROM oc_pr_product WHERE  oc_pr_product.order_id='".$order_id."' LIMIT  1";
		$log->write($sql);
		$query = $this->db->query($sql);
...
public function insert_purchase_order($data = array()){
        $log=new Log('pr_po_order-'.date('Y-m-d').".log");
        ...
        {
			$sql1='INSERT INTO oc_pr_order (order_date,user_id,pre_supplier_bit,store_id,store_type,potential_date) VALUES("' . date('Y-m-d') . '",'.$this->session->data['user_id'].',1,'.$data['stores'][$i][0].',"'.$data['store_type'].'","'.$data['potentialdate'].'")';
            $log->write($sql1);
			$this->db->query($sql1);
        ...
        {
			$sql2='INSERT INTO oc_pr_order (order_date,user_id,store_id,store_type,potential_date) VALUES("' . date('Y-m-d') . '",'.$this->session->data['user_id'].','.$data['stores'][$i][0].',"'.$data['store_type'].'","'.$data['potentialdate'].'")';
            $log->write($sql2);
			$this->db->query($sql2);
            $order_id = $this->db->getLastId();
        }
        ...
        //insert product details
                        ...
                        $p_sql="INSERT INTO oc_pr_product (product_id,name,quantity,order_id,store_id,store_name)    VALUES(".$data['products'][$i][0].",'".$data['products'][$i][1] . "'," . $data['quantity'][$i].",".$order_id.",'".$data['stores'][$i][0]."','".$data['stores'][$i][1]."')";
                        $this->db->query($p_sql);
                        $log->write($p_sql);
                        $product_ids[$i] = $this->db->getLastId();
                           $log->write("price value to check");
                       $log->write($data['price']) ;
                     
                       
                        if($data['supplier_id'] != "--Supplier--")
                        {
                           $sql_pr = "INSERT INTO oc_pr_receive_details (quantity,product_id,supplier_id,order_id,store_id,quantity,price) VALUES(".$data['quantity'][$i].",".$data['products'][$i][0].",".$data['supplier_id'].",".$order_id.",'".$data['stores'][$i][0]."')";
                             $query = $this->db->query($sql_pr);
                             $log->write($sql_pr);
                            }
                            else
                            {
                              $log->write($data['price'][$i]);
                             $sql_price ="INSERT INTO oc_pr_receive_details (product_id,supplier_id,order_id,store_id,quantity,price) VALUES(".$data['products'][$i][0].",-1,".$order_id.",".$data['stores'][$i][0].",".$data['quantity'][$i][0].",".$data['price'][$i].")";
                             $query = $this->db->query($sql_price);
                             $log->write($sql_price);
...
/*
	public function insert_purchase_order($data = array()){
		...
		if($data['supplier_id'] != "--Supplier--")
		{
			for($i = 0; $i<count($data['products']); $i++)
			{
				$query = $this->db->query("INSERT INTO oc_pr_receive_details (quantity,product_id,supplier_id,order_id,store_id) VALUES(".$data['quantity'][$i].",".$data['products'][$i][0].",".$data['supplier_id'].",".$order_id.",'".$data['stores'][$i][0]."')");
			}
		}
		else{
			for($i = 0; $i<count($data['products']); $i++)			{
				$query = $this->db->query("INSERT INTO oc_pr_receive_details (product_id,supplier_id,order_id,store_id) VALUES(".$data['products'][$i][0].",-1,".$order_id.",'".$data['stores'][$i][0]."')");
			}
		}
...
public function getListRec($start,$limit)
	{
		$query = $this->db->query("SELECT
			...
			LEFT JOIN oc_po_supplier 
				ON (oc_pr_receive_details.supplier_id = oc_po_supplier.id)
			INNER JOIN ".DB_PREFIX."user 
				ON (oc_pr_order.user_id = ".DB_PREFIX."user.user_id) WHERE  oc_pr_order.order_sup_send <> '0000-00-00' AND oc_pr_order.delete_bit = 1 AND  oc_pr_order.receive_bit = 0 and pre_supplier_bit=1 GROUP BY oc_pr_order.id ORDER BY oc_pr_order.id DESC LIMIT " . $start . "," . $limit);
...
public function getList_order($oid)
{
	...
	$sql="SELECT receive_bit FROM oc_pr_order where id='".$oid."'";
	$log->write($sql);
	$query = $this->db->query($sql); 
...
public function getList_pending_order($store_id,$product_id)
{
	$log=new Log("stockhis-".date('Y-m-d').".log");
	$sql="SELECT receive_bit FROM oc_pr_order where receive_bit='0' and receive_date='0000-00-00' and order_status_id='1' and store_id='".$store_id."'";
	$log->write($sql);
	$query = $this->db->query($sql); 
	return $query->rows;
...
public function getListRecStore($uid,$start,$limit,$orderstatus=0)
	{

				$log=new Log("inv-".date('Y-m-d').".log");
                                $sdata="";
                                if(!empty($orderstatus))
                                {$sdata=" order_status_id='".$orderstatus."' AND ";}
                                    
		$sql="SELECT
			oc_pr_order.*
			,'PO' as 'receivetype'
                        ,oc_pr_product.name
                        ,oc_pr_receive_details.price
                        ,oc_pr_receive_details.quantity                         
                        FROM
			oc_pr_order
			INNER JOIN oc_pr_receive_details
				ON (oc_pr_order.id = oc_pr_receive_details.order_id)
                         LEFT JOIN oc_pr_product
				ON (oc_pr_order.id = oc_pr_product.order_id)        
			LEFT JOIN oc_po_supplier 
				ON (oc_pr_receive_details.supplier_id = oc_po_supplier.id)
			INNER JOIN ".DB_PREFIX."user 
				ON (oc_pr_order.user_id = ".DB_PREFIX."user.user_id) WHERE  ". $sdata." oc_pr_order.user_id='".$uid."' AND oc_pr_order.order_sup_send = '0000-00-00' AND oc_pr_order.delete_bit = 1 AND  oc_pr_order.receive_bit = 0 and pre_supplier_bit=0 GROUP BY oc_pr_order.id ORDER BY oc_pr_order.id DESC LIMIT " . $start . "," . $limit;
                //echo $sql;
                //exit;
                $log->write($sql);
		$query = $this->db->query($sql);
	...
	public function getList($start,$limit)
	{
		$query = $this->db->query("SELECT
			...
			LEFT JOIN oc_po_supplier 
				ON (oc_pr_receive_details.supplier_id = oc_po_supplier.id)
			INNER JOIN ".DB_PREFIX."user 
				ON (oc_pr_order.user_id = ".DB_PREFIX."user.user_id) WHERE oc_pr_order.delete_bit = 1 GROUP BY oc_pr_order.id ORDER BY oc_pr_order.id DESC LIMIT " . $start . "," . $limit);
		
		return $query->rows;
	}
	public function getTotalOrders()
	{
		$query = $this->db->query("SELECT COUNT(id) as total_orders FROM oc_pr_order WHERE delete_bit = " . 1);
		$results = $query->row;
		return $results['total_orders'];
	}
	public function view_order_details($order_id)
	{
		$query = $this->db->query("SELECT oc_pr_order.*,".DB_PREFIX."user.firstname,".DB_PREFIX."user.lastname
				FROM oc_pr_order
		...
		//ON (oc_pr_receive_details.product_id = oc_pr_product.id)
		$query = $this->db->query("SELECT
		oc_pr_product.*,oc_pr_receive_details.quantity as rd_quantity,oc_pr_receive_details.price,oc_po_supplier.first_name,oc_po_supplier.last_name,oc_po_supplier.id as supplier_id,oc_pr_receive_details.order_id
		FROM
			oc_pr_receive_details
		INNER JOIN oc_pr_product 
			ON (oc_pr_receive_details.order_id = oc_pr_product.order_id)
		LEFT JOIN oc_po_supplier 
			ON (oc_pr_receive_details.supplier_id = oc_po_supplier.id)
				WHERE (oc_pr_receive_details.order_id =".$order_id.")");
	...
	{
		$query = $this->db->query("SELECT * FROM oc_pr_product WHERE order_id = " . $order_info['id']);
		$products = $query->rows;
	}
		$i = 0;
	...
	public function delete($ids)
	{
		...
		foreach($ids as $id)
		{
			if($this->db->query("UPDATE oc_pr_order SET delete_bit = " . 0 ." WHERE id = " . $id))
				$deleted = true;
		}
		if($deleted)
		{
	...
	public function filterCount($filter)
	{
		...
		$query = "SELECT
			...
			INNER JOIN oc_po_supplier 
				ON (oc_pr_receive_details.supplier_id = oc_po_supplier.id)
			INNER JOIN ".DB_PREFIX."user 
				ON (oc_pr_order.user_id = ".DB_PREFIX."user.user_id) WHERE oc_pr_order.delete_bit = 1";
		
		if(isset($filter['filter_id']))
		{
			$query = $query . " AND oc_pr_order.id = " . $filter['filter_id'];
		}
		...
		$query = $query . " GROUP BY (oc_pr_order.id) ORDER BY oc_pr_order.id DESC";
	...
	public function filter($filter,$start,$limit){
		...
		$query = $query . " GROUP BY (oc_pr_order.id) ORDER BY oc_pr_order.id DESC LIMIT ". $start ."," . $limit;
...
$log->write("sql-".$query);
        ...
        //change order status
         public function change_order_status($order_id,$order_status)
          {  
             $log=new Log("change_oder_m.log");
            $sql ="UPDATE `shop`.`oc_pr_order` SET `order_status_id`='".$order_status."' WHERE `id`='".$order_id."'";
            $log->write($sql);
            $query = $this->db->query($sql);
            return $query->rows;
         }
	...
	$log=new Log("receiveorder-".date('Y-m-d').".log");
		...
		$quantities = array();
		$quantity = 0;
		$this->db->query("UPDATE oc_pr_order SET receive_date = '" .$received_order_info['order_receive_date']."', receive_bit = " . 1 . ", pending_bit = " . 0 . " WHERE id = " . $order_id);
		...
		if(count($received_order_info['received_quantities']) != count($received_order_info['suppliers_ids']))
		{
			...
			$log->write("in for loop");
				...
				$log->write("UPDATE oc_pr_receive_details SET price =" .$prices[$i]. ", quantity = ".$received_quantities[$i]." WHERE product_id =".$received_order_info['received_product_ids'][$i] . " AND order_id = " . $order_id);
				$this->db->query("UPDATE oc_pr_receive_details SET  quantity = ".$received_quantities[$i]." WHERE product_id =".$received_order_info['received_product_ids'][$i] . " AND order_id = " . $order_id);
				$query = $this->db->query("SELECT quantity FROM oc_pr_receive_details WHERE product_id = " . $received_order_info['received_product_ids'][$i] . " AND order_id =" . $order_id);
		...
		{
			$query = $this->db->query("SELECT * FROM oc_pr_receive_details WHERE order_id=".$order_id);
					$log->write("update order info done select ".$query);
			if(count($query->rows) > 0)
			{
				$this->db->query("DELETE FROM oc_pr_receive_details WHERE order_id=".$order_id);
			}
			...
			for($j = 0; $j<count($received_order_info['received_product_ids']); $j++)
			{
				for($k = 0; $k<$inner_loop_limit; $k++)
				{
					...
					{
						//"INSERT INTO oc_pr_receive_details (quantity,price,product_id,supplier_id,order_id) VALUES(".$received_order_info['received_quantities'][$k].",".$received_order_info['prices'][$k].",".$received_order_info['received_product_ids'][$j].",".$received_order_info['suppliers_ids'][$k].",".$order_id.")"
						$this->db->query("INSERT INTO oc_pr_receive_details (quantity,product_id,supplier_id,order_id,store_id,price) VALUES(".$received_order_info['received_quantities'][$k].",".$received_order_info['received_product_ids'][$j].",".$received_order_info['suppliers_ids'][$k].",".$order_id.",".$query->rows[$j]["store_id"].",".$received_order_info['price'][$k].")");
		...
		{
			$query = $this->db->query("SELECT DISTINCT product_id FROM oc_pr_product WHERE product_id = " . $received_order_info['received_product_ids'][$i]);
			$product_ids[$i] = $query->row;
			$query1 = $this->db->query("UPDATE oc_pr_product SET received_products = " . $quantities[$i] . " WHERE product_id = " . $received_order_info['received_product_ids'][$i] . " AND order_id = " . $order_id);
		}
					$totalamount=0;
	...
	/*-----------------------------insert receive order function ends here-----------------*/
      public function getstock($product_id,$store_id)
       {
        //$log=new Log("getstock.log");

	$sql = "SELECT * FROM shop.oc_product_to_store where product_id='".$product_id."' and store_id='".$store_id."'";
        //$log->write($sql);
        $query = $this->db->query($sql);
        //$log->write($query );      
        return $query->rows;
      ...
      {
          //$log=new Log("getstorename.log");
          $sql="SELECT shop.oc_store.name FROM shop.oc_store where store_id='".$store_id."'";
          //$log->write($sql);
          $query = $this->db->query($sql);
          //$log->write($query);
          return $query->rows;
