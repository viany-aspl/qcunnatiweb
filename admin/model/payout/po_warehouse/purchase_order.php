...
public function insert_purchase_order($data = array()){
        ...
        {
			$sql1='INSERT INTO oc_po_order (order_date,user_id,pre_supplier_bit,store_id,store_type,potential_date,po_number,spo_po_linked_status ) VALUES("' . date('Y-m-d') . '",'.$this->session->data['user_id'].',1,'.$data['stores'][$i][0].',"'.$data['store_type'].'","'.$data['potentialdate'].',"'.$data['po_number'].', \'1\'")';
            $log->write($sql1);
			$this->db->query($sql1);
        ...
        {
			$sql2='INSERT INTO oc_po_order (order_date,user_id,store_id,store_type,potential_date,po_number,spo_po_linked_status) VALUES("' . date('Y-m-d') . '",'.$this->session->data['user_id'].','.$data['stores'][$i][0].',"'.$data['store_type'].'","'.$data['potentialdate'].',"'.$data['po_number'].', \'1\'")';
            $log->write($sql2);
			$this->db->query($sql2);
            $order_id = $this->db->getLastId();
        }
	...
	public function upload_pr_document($data) 
        {
            ...
            $sql2="update oc_po_order SET pr_varification_image='".$data['file']."' where id='" .$data['order_id']. "'";
            $log->write($sql2);
