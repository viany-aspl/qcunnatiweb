...
error_reporting(0);
class ControllerPrApprovePurchaseOrder extends Controller{
        public function check_ware_house_quantity()
        {
            ...
            for($a=0;$a<$total_product;$a++)
            {
               ...
               $this->load->model('pr_approve/purchase_order');
               $data_qnty=$this->model_pr_approve_purchase_order->check_ware_house_quantity($ware_house,$product_id,$p_qnty);
	...
	public function index()
	{
		//set the title of the page
			$this->document->setTitle("PR Approve");
			...
			$data['breadcrumbs'][] = array(
			'text' => "Purchase Request Zonal",
			'href' => $this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('pr_approve/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('pr_approve/purchase_order/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['filter'] = $this->url->link('pr_approve/purchase_order/filter', 'token=' . $this->session->data['token'] . $url, true);
		...
		$order_list = $this->model_pr_approve_purchase_order->getList($filter_data);
                
                
               // $order_list = $this->model_pr_approve_purchase_order->conform_order();
		//$data['order_list']=array();
                ...
                foreach($order_list as $order)
                {
                    ...
                    $order_id=$order['id'];
                     $order_information= $this->model_pr_approve_purchase_order->view_order_details($order_id);
		...
		//print_r($order_list);
		$total_orders = $this->model_pr_approve_purchase_order->getTotalOrders($filter_data);
		
		//getting total orders
		$data['view'] = $this->url->link('pr_approve/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		$data['receive'] = $this->url->link('pr_approve/confirm_dispatch', 'token=' . $this->session->data['token'] . $url, true);
		...
		$pagination->url = $this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['my_custom_text'] = "This is pr_approve order page.";
		$this->response->setOutput($this->load->view('pr_approve/purchase_order_list.tpl', $data));
	...
	public function download_excel()
	{
		...
		$order_list = $this->model_pr_approve_purchase_order->getList($filter_data);
		//$results=$this->model_pr_approve_purchase_order->download_excel($filter_data);
...
echo '</tbody>
	...
	public function view_order_details()
	{
		...
		$this->load->model('pr_approve/purchase_order');
		$data['order_information'] = $this->model_pr_approve_purchase_order->view_order_details($order_id);
		//print_r($data['order_information']);
                $data['cancel'] = $this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['pdf_export'] = $this->url->link('pr_approve/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		if(isset($_GET['export']))
		{
			...
			$html = $this->load->view('pr_approve/print_order.tpl',$data);
		...
		{
			$this->response->setOutput($this->load->view('pr_approve/view_order.tpl',$data));
		}
	}
	...
	public function update_order()
        {
            $log =new Log("update_order-".date('Y-m-d').".log");
            $log->write($this->request->get);
            $log->write($this->request->post);
            $order_id = $this->request->post['order_id'];
            $data['order_id'] = $order_id;
            $this->load->model('pr_approve/purchase_order');
            $result=$this->model_pr_approve_purchase_order->update_order($data);
	...
	public function receive_order()
	{
		...
		$url = '';
			...
			if (isset($this->request->get['page'])) {
                ...
                if($this->request->server['REQUEST_METHOD'] == 'POST')
                {
                    ...
                    $inserted = $this->model_pr_approve_purchase_order->insert_receive_order($this->request->post,$order_id);
                    $this->session->data['success'] = 'Order Accepeted successfully';
                    $this->response->redirect($this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		...
		$data['action'] = $this->url->link('pr_approve/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$this->load->model('pr_approve/supplier');
		$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
		//print_r($data['suppliers']);
                	//$data['ware_houses']=$this->model_pr_approve_purchase_order->get_ware_houses();
                //print_r($data['order_information']);//['order_info']['user_id']
                $data['user_id']=$data['order_information']['order_info']['user_id'];
		$this->response->setOutput($this->load->view('pr_approve/receive_order.tpl',$data));
	...
	public function insert_receive_order()
	{
		...
		if((count($received_quantities) != count(array_filter($received_quantities))) || (count($prices) != count(array_filter($prices))) || $order_receive_date == '')
		{
			...
			$data['action'] = $this->url->link('pr_approve/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$this->load->model('pr_approve/supplier');
			$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
			//$this->response->redirect($this->url->link('pr_approve/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true));
			$this->response->setOutput($this->load->view('pr_approve/receive_order.tpl',$data));
		}
		else
		{
			...
			$this->load->model('pr_approve/purchase_order');
			$inserted = $this->model_pr_approve_purchase_order->insert_receive_order($received_order_info,$order_id);
			if($inserted)
			{
				$_SESSION['receive_success_message'] = 'Order received Successfully!!';
				$this->response->redirect($this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, true));
			}
			else
			{
				$_SESSION['something_wrong_message'] = 'Sorry!! something went wrong, try again';
				$this->response->redirect($this->url->link('pr_approve/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true));
			}
	...
	/*--------------------Insert pr_approve Indent starts heres-------------------------------------------------*/
	...
	public function insert_purchase_order()
	{
		...
		if((in_array("--products--",$data['products'])) || (in_array("--stores--",$data['storess'])) || (in_array("--Product Options--",$data['options'])) || (in_array("--Option Values--",$data['option_values'])) || (count($data['quantity']) != count(array_filter($data['quantity']))) || (count($data['options']) != count(array_filter($data['options']))) || (count($data['products']) != count(array_filter($data['products']))) || (count($data['option_values']) != count(array_filter($data['option_values']))))
		{
			...
			$url = '';
			$data['action'] = $this->url->link('pr_approve/purchase_order/insert_purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('pr_approve/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$this->load->model('pr_approve/supplier');
			$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
		                //stores
		...
		$data['stores'] = $this->model_setting_store->getStores();  
			$this->load->model('catalog/option');
			$data['options'] = $this->model_catalog_option->getOptions();
			$this->response->setOutput($this->load->view('pr_approve/purchase_order_form.tpl', $data));
			//$this->response->redirect($this->url->link('pr_approve/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true));
		}
		else
		{
			$i = 0;
			...
			$this->load->model('pr_approve/purchase_order');
			$order_id = $this->model_pr_approve_purchase_order->insert_purchase_order($data);
			...
			if(isset($this->request->post['mail_bit']))
			{
				...
				$html = $this->load->view('pr_approve/mail_purchase_order.tpl',$data);
	...
	/*--------------------Insert pr_approve order ends here----------------------------*/
}
