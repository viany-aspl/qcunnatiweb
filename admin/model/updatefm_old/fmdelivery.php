...
class ModelUpdatefmFmdelivery extends Model {
...
WHERE od.invoice_no <> '' AND od.fmcode <> '0' ";
    ...
    public function getfm($inv) {
          $sql = "SELECT DISTINCT(fmcode),fmname from oc_order_delivery ";
    ...
    public function getallfm() {
        ...
        $sql = "SELECT fmcode,fmname from oc_order_delivery where invoice_no='" . $inv . "' ";
        $query = $this->db->query($sql);

        return $query->rows;
    }
    public function updatefmname($fm_code,$fm_name,$inv) {

        $sql = "UPDATE `oc_order_delivery` SET `fmcode`='".$fm_code."', `fmname`='".$fm_name."' WHERE `invoice_no`='".$inv."'; ";

        $query = $this->db->query($sql);
        //echo $sql;
        if($query){
            $msg = array('status'=>'success','responce'=>'FM has been updated.');
        }else{
            $msg = array('status'=>'error','responce'=>'Fm not updated. Please try again.');            
        }
