<?php
class ModelCatalogStorecredit extends Model {
...
//new
	public function addProduct($data) {
		$this->event->trigger('pre.admin.product.add', $data);
                ...
                foreach ($storeprd as $store_id) {
                    ...
                    if($data['quantitystoremin'.$store_id['store_id']]=='')
                    {
                        $data['quantitystoremin'.$store_id['store_id']]=0;
                    }
                    $data['minquantity']+=$data['quantitystoremin'.$store_id['store_id']];
                    
                }
                
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET HSTN = '" . $this->db->escape($data['hstn']) . "',model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', minquantity = '" . (int)$data['minquantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
		...
		if (isset($data['product_store'])) 
                    {
			foreach ($storeprd as $store_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_to_store SET product_id = '" . (int)$product_id . "', store_id = '" . (int)$store_id['store_id'] . "',minquantity = '" . (int)$data['quantitystoremin'.$store_id['store_id']] . "',store_price='".(float)$data['quantitystorefactor'.$store_id['store_id']]."'");
			}
		}
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
					if (isset($product_option['product_option_value'])) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', minquantity = '" . (int)$product_option_value['minquantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
						}
					}
				} else {
	...
	public function editProduct($product_id, $data) 
	{
		...
		$log=new Log("product-credit-".date('Y-m-d').".log");		
                $data['minquantity']=0;
              ...
              /*  foreach ( $storeprd   as $store_id) 
				...
				{ 
					...
					. "factor='". $data['quantitystorefactor'.$store_id['store_id']]."',"
					."minquantity='". ($data['quantitystoremin'.$store_id['store_id']])."',"
                                        ."maxquantity='". ($data['qquantitystoremax'.$store_id['store_id']])."' "        
					." ON DUPLICATE KEY
					UPDATE "
					. "factor='".$data['quantitystorefactor'.$store_id['store_id']]."'"
					. ",maxquantity='". ($data['qquantitystoremax'.$store_id['store_id']])."',"
					...
					;
                                         ...
                                         $log->write($sql_p);
                                        $this->db->query($sql_p);
                                        $all_quantity=$all_quantity+$data['quantitystoremin'.$store_id['store_id']];      
                                        $price_withouttax=$data['quantitystorefactor'.$store_id['store_id']];				
				
				}
                /*if($data['user_group_id'] =="35")
				{    
					$upd_q="UPDATE " . DB_PREFIX . "product SET minquantity = '" . (int)$all_quantity. "' WHERE product_id = '" . (int)$product_id . "'";
                }*/
	...
	public function editProductIsec($product_id, $data) 
	{
				...
				$sql_p=" INSERT INTO oc_product_to_store SET product_id='".$product_id."', "
					...
					. "store_price='". $data['quantitystorefactor'.$store_id['store_id']]."',"
					." store_tax_amt='". ($data['qquantitystoremax'.$store_id['store_id']]-$data['quantitystorefactor'.$store_id['store_id']])."' "
					." ON DUPLICATE KEY
					UPDATE "
					. "store_price='".$data['quantitystorefactor'.$store_id['store_id']]."'"
					. ",store_tax_amt='". ($data['qquantitystoremax'.$store_id['store_id']]-$data['quantitystorefactor'.$store_id['store_id']])."' "
				...
				$log_q="insert into  oc_product_price_log SET product_id = '" . $product_id. "',store_id = '" . $store_id['store_id'] . "',old_price = '" . $old_price . "',new_price = '" . $data['quantitystorefactor'.$store_id['store_id']] . "',user_id='".$this->user->getId()."'";
				$log->write($log_q);
				$this->db->query($log_q);
				}
				/*
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			foreach ($product_option_value_query->rows as $product_option_value) {
				$product_option_value_data[] = array(
					...
					'minquantity'                => $product_option_value['minquantity'],
	...
	public function getProductDiscounts($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$product_id . "' ORDER BY minquantity, priority, price");
	...
	public function getProductStores($product_id) {
        ...
        public function getProductStoresQuantity($product_id) {
            ...
            //$product_store_data = array();
		$query = $this->db->query("SELECT *,tax_class_id FROM " . DB_PREFIX . "product_to_store p2s LEFT JOIN oc_product on oc_product.product_id=p2s.product_id  WHERE p2s.product_id = '" . (int)$product_id . "'");
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_store as prd LEFT JOIN " . DB_PREFIX . "store as st on st.store_id=prd.store_id WHERE product_id = '" . (int)$product_id . "'");		
                foreach ($query->rows as $result) {
                    ...
                    $product_store_data=$product_store_data.$storename."-".$result['minquantity']."<br/>";                        
		}
...
public function getProductsRelatedToCrop($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) join oc_product_related as opr on opr.product_id=p.product_id WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' and opr.related_id='".$data["filter_crop_id"]."' ";
