...
//new
	...
	public function editProduct($product_id, $data) 
	{
				...
				$sql_p=" INSERT INTO oc_product_to_store SET product_id='".$product_id."', "
					...
					." ON DUPLICATE KEY
					UPDATE "
					. "store_price='".number_format((float)($data['quantitystoreprice'.$store_id['store_id']]), 2, '.', '')."'"
					. ",store_tax_amt='". ($data['qquantitystoreprice'.$store_id['store_id']]-$data['quantitystoreprice'.$store_id['store_id']])."' "
					;
				...
				$log_q="insert into  oc_product_price_log SET product_id = '" . $product_id. "',store_id = '" . $store_id['store_id'] . "',old_price = '" . $old_price . "',new_price = '" . $data['quantitystoreprice'.$store_id['store_id']] . "',user_id='".$this->user->getId()."'";
				$log->write($log_q);
				$this->db->query($log_q);
				}
