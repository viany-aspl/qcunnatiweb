...
//		$log->write($i);
        ...
        {
			$sql1='INSERT INTO oc_po_order (order_date,user_id,pre_supplier_bit,store_id,store_type,potential_date,po_number,spo_po_linked_status ) VALUES("' . date('Y-m-d') . '",'.$this->session->data['user_id'].',1,'.$data['stores'][0].',"'.$data['store_type'].'","'.$data['potentialdate'].'","'.$data['po_number'].'", \'1\')';
            $log->write($sql1);
			$this->db->query($sql1);
        ...
        {
			$sql2='INSERT INTO oc_po_order (order_date,user_id,store_id,store_type,potential_date,po_number,spo_po_linked_status) VALUES("' . date('Y-m-d') . '",'.$this->session->data['user_id'].','.$data['stores'][0].',"'.$data['store_type'].'","'.$data['potentialdate'].'","'.$data['po_number'].'", \'1\')';
            $log->write($sql2);
			$this->db->query($sql2);
            $order_id = $this->db->getLastId();
        }
       ...
       $this->db->query($sql3);
        ...
        //insert product details
            ...
            $product_ids[0] = $this->db->getLastId();
                        ...
                        $log->write("INSERT INTO oc_po_receive_details (quantity,product_id,supplier_id,order_id,store_id) VALUES(".$data['quantity'].",".$data['products'][0].",".$data['supplier_id'].",".$order_id.",'".$data['stores'][0]."')");
                            }
                            else
                            {
                ...
                $query = $this->db->query("INSERT INTO oc_po_receive_details (product_id,supplier_id,order_id,store_id) VALUES(".$data['products'][0].",-1,".$order_id.",'".$data['stores'][0]."')");
                        ...
                        $log->write("INSERT INTO oc_po_receive_details (product_id,supplier_id,order_id,store_id) VALUES(".$data['products'][0].",-1,".$order_id.",'".$data['stores'][0]."')");
                            }
