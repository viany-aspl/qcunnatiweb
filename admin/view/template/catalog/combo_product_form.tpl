...
<?php if ($config_tax_included) { //echo $config_tax_included; ?>
			<div class="form-group">
...
<div style="display: none;">
            ...
            <div class="tab-pane" id="tab-links">
			  ...
			  <div class="form-group">
                <label class="col-sm-2 control-label" for="input-linked"><span data-toggle="tooltip" title="Select the product included in Combo">Linked Products</span></label>
                <div class="col-sm-10">
                  <input type="text" name="linked" value="" placeholder="Linked Products" id="input-linked" class="form-control" />
                  <div id="product-linked" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($linked_products as $linked_product) 
						{ 
						//print_r($linked_product);
						?>
                    <div id="product-linked<?php echo $linked_product['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $linked_product['name']; ?> (<?php echo $linked_product['quantity']; ?>)
							<input type="hidden" name="linked_product[]" value="<?php echo $linked_product['product_id']; ?>" />
							<input type="hidden" name="linked_qnty[]" value="<?php echo $linked_product['quantity']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
                </div>
...
<!-- Modal -->
  <div class="modal fade" id="myModal_linked" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Quantity (<span id="span_label"></span>)</h4>
        </div>
        <div class="modal-body">
        
           <input type="hidden" name="input_value" id="input_value" value="" /> 
		   <input type="hidden" name="input_label" id="input_label" value="" /> 
			<div class="form-group">
                <label class="control-label" for="input-date">Quantity</label>
                
                  <input required type="text" name="input_qnty" onkeypress='validate(event)' placeholder="Quantity"  id="input_qnty" class="form-control" />
                  
                 
              </div>
            
            <div class="text-right">
                <input type="button" id="partner_sbmt_btn" onclick="return submit_qnty();"  class="btn btn-primary" value="Submit" />
                <button type="button" id="partner_cncl_btn" class="btn btn-default" onclick="return cncl_qnty()" data-dismiss="modal">Cancel</button>
            </div>
...
$('input[name=\'linked\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
	...
	'select': function(item) 
	{
		open_qnty_modal(item['value'],item['label']);
	}
});
function open_qnty_modal(value,label)
{
	$('#myModal_linked').modal('show');
	$("#input_value").val(value);
	$("#input_label").val(label);
	$("#span_label").html(label);
	$("#input_qnty").val('');
	
}
function submit_qnty()
{
	var qnty=$("#input_qnty").val();
	var value=$("#input_value").val();
	var label=$("#input_label").val();
	//alert(qnty);
	if(qnty)
	{
	$('input[name=\'linked\']').val('');
		
		$('#product-linked' + value).remove();
		
		$('#product-linked').append('<div id="product-linked' + value + '"><i class="fa fa-minus-circle"></i> ' + label +' ('+qnty+') ' +'<input type="hidden" name="linked_product[]" value="' + value + '" />'+'<input type="hidden" name="linked_qnty[]" value="' + qnty + '" /></div>');	
		$('#myModal_linked').modal('hide');
		return true;
	}
	else
	{
		alertify.error('Please enter quantity required for Combo Pack');
		return false;
	}
...
$('#product-linked').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
// Category
