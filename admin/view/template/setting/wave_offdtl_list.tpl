...
<i class="<?php echo $tool_tip_class; ?> " data-toggle="tooltip" style="<?php echo $tool_tip_style; ?>" title="<?php echo $tool_tip; ?>"></i>
  ...
  <div class="container-fluid">
	...
	<td class="text-left">Approved By</td>
				<?php if(($user_group_id==1) || ($user_group_id==13)){ ?>
	...
	<td class="text-left"><?php echo $pay['store_user']; ?></td>
	 ...
	 <?php if(($user_group_id==1) || ($user_group_id==13)){ ?>
			<td class="text-left" id="td_<?php echo $pay['id']; ?>">
				<img id="sbmt_img_<?php echo $pay['id']; ?>" src="view/image/processing_image.gif" style="height: 48px;width: 48px;display: none;" />
				<a title="Cancel this Transaction !" href="#" id="submit_btn_<?php echo $pay['id']; ?>" 
				class="btn btn-primary pull-right" onclick="return cancel_exp_trans(<?php echo $pay['id']; ?>,<?php echo $pay['store_user_id']; ?>);">
					Cancel 
				</a>
			</td>
	 <?php } ?>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <form action="index.php?route=setting/debitstore/exp&token=<?php echo $token; ?>" method="get" enctype="multipart/form-data" >
            ...
            <select name="store_id" style="width:100%;" id="input-store_id" class="form-control" required onchange="return get_store_incharge(this.value);" >
	    <option value="">SELECT STORE</option>
            ...
            <div class="form-group">
            <label for="input-username">STORE IN-CHARGE/RUNNER</label>
            <div class="input-group"><span class="input-group-addon"><i class="fa fa-building-o" aria-hidden="true"></i></span>
                
            <select name="store_user_id" id="input-store_user_id" class="form-control" required >
	    <option value="">SELECT IN-CHARGE/RUNNER</option>
...
function cancel_exp_trans(order_id,store_user_id)
{
	//alert(order_id);
	var cnfrm=confirm('Are you sure ? You want to cancel this transaction !');
	if(cnfrm)
	{
	url = 'index.php?route=setting/debitstore/cancel_exp_trans&token=<?php echo $token; ?>&order_id='+order_id+'&store_user_id='+store_user_id;
	$.ajax({
              url: url,
              // dataType: 'json',
			  beforeSend: function() 
			  {
               ...
               success: function(json) 
               {
					...
					if(json=='1')
					{
						alertify.success('Transaction Cancled successfully');
						$("#submit_btn_"+order_id).hide();
