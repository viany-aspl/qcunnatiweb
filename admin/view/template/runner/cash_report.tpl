...
<div id="content">
  ...
  <div class="container-fluid">
	...
	<td class="text-right"><?php if($order['status']=="0"){ ?>
<?php if($order['bank_id']=='6'){ ?>
            <a class="btn btn-primary" onclick="return confirm_box_mpesa('<?php echo $order['tr_id']; ?>','<?php echo $logged_user; ?>','<?php echo $order['amount']; ?>');">Accept</a> 
            <?php }else{ ?>
            <a class="btn btn-primary" onclick="return confirm('Are you sure ? You want to confirm the amount !');" href="index.php?route=runner/cash_report/accept_cash&token=<?php echo $token; ?>&tr_id=<?php echo $order['tr_id']; ?>&logged_user=<?php echo $logged_user; ?>&amount=<?php echo $order['amount']; ?>">Accept</a> 
            <?php } ?>

                       <br/>  <br/>
                        <a class="btn btn-primary" style="width: 66px;background-color: #B36330;" onclick="return confirm('Are you sure ? You want to reject the amount !');" href="index.php?route=runner/cash_report/reject_cash&token=<?php echo $token; ?>&tr_id=<?php echo $order['tr_id']; ?>&logged_user=<?php echo $logged_user; ?>&amount=<?php echo $order['amount']; ?>">Reject</a> <?php } else {  } ?></td>
              </tr>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Please enter the M-PESA transaction number </h4>
        </div>
        <div class="modal-body">
        <form action="index.php?route=runner/cash_report/accept_cash_mpesa&token=<?php echo $token; ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
            <label for="input-username">Transaction number </label>
            <div class="input-group"><span class="input-group-addon"><i class="fa fa-envelope"></i></span>
                <textarea name="m_pesa_tr_number" required id="m_pesa_tr_number" class="form-control" placeholder="Transaction number "></textarea>
            </div>
            </div>
...
function confirm_box_mpesa(tr_id,logged_user,amount)
   {
    $('#bill_id').val(tr_id);
    $('#amount').val(amount);
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=runner/cash_report&token=<?php echo $token; ?>';
...
$('#button-download').on('click', function() {
    url = 'index.php?route=runner/cash_report/download_excel&token=<?php echo $token; ?>';
