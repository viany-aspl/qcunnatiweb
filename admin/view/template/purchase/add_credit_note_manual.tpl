...
<div id="content">
  ...
  <h1><?php echo "Credit Note (Manual)"; ?></h1>
      <ul class="breadcrumb">
        <?php foreach ($breadcrumbs as $breadcrumb) { ?>
        <li><a href="<?php echo $breadcrumb['href']; ?>">Credit Note (Manual)  </a></li>
        <?php } ?>
      </ul>
							...
							<td class="col-sm-6">
								<div class="col-sm-6">
									<div class="form-group">
										<label class="control-label" for="input-date-end"><?php echo "Purchase Invoice No"; ?></label>
										<input autocomplete="off"  name="invoice_num" id="invoice_num" type="text" placeholder="Invoice No"  class="form-control"/>
									</div>
								</div> 
								<div class="col-sm-6">
									<div class="form-group">
										<label class="control-label" for="input-date-end"><?php echo "PO No"; ?></label>
										<input autocomplete="off"  name="po_no" id="po_no" type="text" placeholder="PO No"  class="form-control"/>
									</div>
								</div> 
								<div class="col-sm-6">
									<div class="form-group">
										<label class="control-label" for="input-date-end"><?php echo "C N./Invoice Number"; ?></label>
										<input autocomplete="off"  name="cn_no" id="cn_no" type="text" placeholder="C N./Invoice Number" required="required" class="form-control"/>
									</div>
								</div> 
								<div class="col-sm-6">
								<div class="form-group">
									<label class="control-label" for="input-date-end"><?php echo "CN Invoice Date"; ?></label>
									<div class="input-group date">
										<input type="text" name="invoice_date" autocomplete="off" readonly="readonly"  value="" placeholder="<?php echo "Invoice Date"; ?>" data-date-format="YYYY-MM-DD" id="invoice_date" class="form-control" required="required"/>
		...
		<table class="table table-bordered" id="print_table" border="1">
          <thead>
                <tr>
                          ...
                          <td class="text-left" style="width: 11.11%;">Quantity</td>
							...
							<td class="text-left" style="width: 11.11%;display: none;">Discount</td>
		  ...
		  ?>
            <tr id="tr_<?php echo  $a; ?>">
			  ...
			  <td class="text-left" style="display: none;" id="td_p_qnty_<?php echo  $a; ?>">
          ...
          <tbody>
                <tr id="tr_scgst_by_18" >
                    ...
                    <td style="width: 22%;" class="text-left">
							...
							<input type="text" autocomplete="off"  required="required" onkeypress="return remove_zero_t(this.value,event);" onkeyup="return update_by_t(this.value);" class="form-control" name="round_off" id="round_off" value="0" />
                    </td>
		</tr>
                <tr id="tr_scgst_by_18" >
                    <td class="text-right" style="width: 78%;"><b>
...
function updatebuttonvalue(data)
{
	...
	if(!cn_no)
	{
		alertify.error('Please Enter CN No');
		$("#cn_no").focus();
		return false;
	}
	if(!invoice_date)
...
{   //alert(valuee);
   ...
   if(valuee=='-.')
    ...
    var total_amount=(((p_price)*(qntty))-parseFloat(discount));
...
function update_by_q(valuee,a)
{
    ...
    $("#span_sgst_type_"+thisid).val('CGST @'+sgst+'% '); 
        ...
        grand_total=parseFloat(total_tax)+parseFloat(sub_total);//
        sub_total=parseFloat(sub_total).toFixed(2);
	var tranport_charge=$("#round_off").val();
		grand_total=parseFloat(grand_total)+parseFloat(tranport_charge);//
        grand_total=parseFloat(grand_total).toFixed(2);
        $('#td_sub_total').html(sub_total);
        $('#sub_total').val(sub_total);
...
{   //alert(valuee);
    ...
    var total_amount=(((p_price)*(qntty))-parseFloat(valuee));
    total_amount=parseFloat(total_amount).toFixed(2);
    ...
    { 
	   ...
	   alertify.error('Discount Should be less then Amount');
		...
		total_amount=(((p_price)*(qntty))-parseFloat(0));
		total_amount=parseFloat(total_amount).toFixed(2);
    } 
	//alert(total_amount);
    $('#p_amount_'+a).val(total_amount);
...
function update_by_price(valuee,a)
{
    ...
    var total_amount=(((valuee)*(qntty))-parseFloat(p_discount));
    total_amount=parseFloat(total_amount).toFixed(2);
    ...
    { 
	   ...
	   alertify.error('Price Should be greater then Discount');
	    ...
	    total_amount=(((valuee)*(qntty))-parseFloat(0));
		total_amount=parseFloat(total_amount).toFixed(2);
    }
	//alert(total_amount);
    $('#p_amount_'+a).val(total_amount);
 ...
 <script type="text/javascript">
   $('input[name=\'product_name[]\']').autocomplete({
	...
	if(!qntty)
	{
        ...
        var total_amount=(((parseFloat(item['price_wo_t']))*(qntty))-parseFloat(p_discount));
        total_amount=parseFloat(total_amount).toFixed(2);
