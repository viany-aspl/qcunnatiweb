...
<?php if ($error) {  ?>
	...
	<tr>                
            ...
            <input id="submit_btn" type="button" style="margin-top: 5px;float: right;display: none;" name="indent_submit" value="Cancel Order" onclick="return submit_order();" class="btn btn-primary"   />
  ...
  <script type="text/javascript">
	...
	var order_status_id=$("#order_status_id").val();
                    ...
                    { 
       alertify.confirm('Are you Sure ? You want to Cancel the Order !',
            function(e)
            { 
                if(e)
                {
    ...
    function send_to_cancel(order_id)
    {
      try{
	  $.ajax({
		url: 'index.php?route=sale/ordercancel/cancel_order&token=<?php echo $token; ?>&order_id=' + encodeURIComponent(order_id),
		dataType: 'json',
		beforeSend: function() 
                {  
                    $("#submit_img").show();
  ...
  function search_order()
  {
	...
	try{
	  $.ajax({
		url: 'index.php?route=sale/ordercancel/search_order&token=<?php echo $token; ?>&order_id=' + encodeURIComponent(order_id),
		dataType: 'json',
		beforeSend: function() {
			$("#search_btn").hide();
			$("#search_img").show();
