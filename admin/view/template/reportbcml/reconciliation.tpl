    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        ...
        <button type="button" id="button-bill" class="btn btn-primary pull-right" onclick="open_model()" style="margin-top: -8px !important; margin-right: 10px !important;">
            Create Bill</button>
			<?php if($user_group_id!=11){  ?>
<button type="button" id="button-bill_check" class="btn btn-primary pull-right" onclick="open_model_check()" style="margin-top: -8px !important; margin-right: 10px !important;">
            Create Bill for Check (Testing)</button>
        <button type="button" id="button-details" class="btn btn-primary pull-right" style="margin-top: -8px !important; margin-right: 10px !important;">
			...
			<?php if($user_group_id==1){  ?>
			 <button type="button" id="button-bill_duplicate" class="btn btn-primary pull-right" style="margin-top: -8px !important; margin-right: 10px !important;">
            Create Duplicate Bill</button>
			<?php } ?>
      ...
      <div class="panel-body">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                <label class="control-label" for="input-date-end">Select Store</label>
                  ...
                  <select <?php if($user_group_id==11){  ?> disabled="true" <?php } ?> name="filter_store" style="width: 100%;" id="input-store" class="select2 form-control">
                      <option value="">SELECT STORE</option>
                  <?php foreach ($stores as $store) { ?>
                  <?php if (($store['store_id'] == $filter_store) || ($user_group_id==11 && $user_store_id==$store['store_id']))
					{ ?>
		...
		<div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <select <?php if($user_group_id==11){  ?> disabled="true" <?php } ?> name="filter_unit" id="input-unit" class="form-control">
                         <option value="" >SELECT UNIT</option>
			<?php foreach($units as $unit) {  ?>
				<option value="<?php echo $unit['unit_id']; ?>" 
				<?php if (($unit['unit_id'] == $filter_unit) || ($user_group_id==11 && $user_unit_id==$unit['unit_id'])) { ?> selected="selected" <?php } ?>>
	...
	<td  class="text-right">Order ID</td>
   	 <td  class="text-right">Inv no.</td>
                ...
                <td class="text-left">Grower Detail</td>
	...
	<td class="text-right"><?php echo $order['inv_no']; ?></td>
                ...
                <td class="text-left"><?php echo $order['farmer_name']; ?>
      ...
      <div class="modal-content">
        ...
        <form action="index.php?route=reportbcml/reconciliation/create_bill&token=<?php echo $token; ?>" method="post" enctype="multipart/form-data" onsubmit="return myFunction()" >
            ...
            <div class="input-group"><span class="input-group-addon"><i class="fa fa-building-o" aria-hidden="true"></i></span>
               ...
               <select  name="filter_store_2" id="input-store1" class="form-control" required onchange="get_unit(this.value)">
	    <option value="">SELECT STORE</option>
                  <?php foreach ($stores as $store) { ?>
                  <?php if (($store['store_id'] == $filter_store) || ($user_group_id==11 && $user_store_id==$store['store_id']))
					{ ?>
            ...
            <div class="input-group"><span class="input-group-addon"><i class="fa fa-mobile"></i></span>
                <select  name="filter_unit_2" id="input-unit_2" class="form-control" required >
                         <option value="" >SELECT UNIT</option>
                                        ...
                                        <?php foreach($units as $unit) {  ?>
				<option value="<?php echo $unit['unit_id']; ?>" <?php if (($unit['unit_id'] == $filter_unit) || ($user_group_id==11 && $user_unit_id==$unit['unit_id'])) { ?> selected="selected" <?php } ?>><?php echo $unit['unit_name']; ?></option> 
			<?php } ?>
...
<!-- Modal check -->
  <div class="modal fade" id="myModal_create_bill_check" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" id="partner_cncl_btn2" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Create Bill For Check</h4>
        </div>
        <div class="modal-body">
        <form action="index.php?route=reportbcml/reconciliation/create_bill_check&token=<?php echo $token; ?>" method="post" enctype="multipart/form-data" onsubmit="return myFunction_check()" >
            <div class="form-group">
                <label class="control-label" for="input-date">Date</label>
                <div class="input-group date" id="date_cr">
                  <input required type="text" name="filter_date_check"  placeholder="" data-date-format="YYYY-MM-DD" id="input-date" class="form-control" />
                  <span class="input-group-btn">
                  <button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button>
                  </span></div>
              </div>
            ...
            <div class="input-group"><span class="input-group-addon"><i class="fa fa-building-o" aria-hidden="true"></i></span>
               ...
               <select <?php if($user_group_id==11){  ?> disabled="true" <?php } ?> name="filter_store_2_check" id="input-store2" class="form-control" required onchange="get_unit_check(this.value)">
	    <option value="">SELECT STORE</option>
                  <?php foreach ($stores as $store) { ?>
                  <?php if (($store['store_id'] == $filter_store) || ($user_group_id==11 && $user_store_id==$store['store_id']))
					{ ?>
            ...
            <label for="input-username">Unit</label>
            <div class="input-group"><span class="input-group-addon"><i class="fa fa-mobile"></i></span>
                <select <?php if($user_group_id==11){  ?> disabled="true" <?php } ?> name="filter_unit_2_check" id="input-unit_2_check" class="form-control" required >
                         <option value="" >SELECT UNIT</option>
  ...
  <?php if($user_group_id==11){  ?>
  $('#input-store1 option:not(:selected)').attr('disabled', true);
  $('#input-store2 option:not(:selected)').attr('disabled', true);
  $('#input-unit_2 option:not(:selected)').attr('disabled', true);
  $('#input-unit_2_check option:not(:selected)').attr('disabled', true);
  <?php } ?>
$("#input-store").select2();
...
function get_unit_check(store_id)
{ //alert(store_id);
url = 'index.php?route=report/reconciliation/get_store_unit&token=<?php echo $token; ?>&store_id='+store_id;
$.ajax({
              url: url,
...
//////////////////////////////////
function open_model_check()
{
$('#myModal_create_bill_check').modal('show');
$('input[name=\'filter_date_check\']').val('');
$('select[name=\'filter_store_2_check\']').val('');
$('select[name=\'filter_unit_2_check\']').val('');

return false;
}
function myFunction_check() {
    
             var filter_date = $('input[name=\'filter_date_check\']').val();
	
              var filter_store = $('select[name=\'filter_store_2_check\']').val();
	
	var filter_unit = $('select[name=\'filter_unit_2_check\']').val();
	
	if ( (filter_date!= '') && (filter_store != '') && (filter_unit != '') ){
		
		$('#myModal_create_bill_check').modal('hide');
...
$('#button-bill_duplicate').on('click', function() {
	url = 'index.php?route=reportbcml/reconciliation/duplicate&token=<?php echo $token; ?>';
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=reportbcml/reconciliation&token=<?php echo $token; ?>';
...
$('#button-pdf').on('click', function() {
	url = 'index.php?route=reportbcml/reconciliation/download_pdf&token=<?php echo $token; ?>';
