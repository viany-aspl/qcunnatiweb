...
<span style="font-weight: bold;">Total C-Tagged : <?php echo number_format((float)$total_cash_tagged_all, 2, '.', ''); ?></span> 
	...
	<span style="font-weight: bold;">Total C-Subsidy : <?php echo number_format((float)$total_subsidy_all, 2, '.', ''); ?></span> 
          &nbsp; | &nbsp;
 <span style="font-weight: bold;">Total Tagged : <?php echo number_format((float)$total_bcml_tagged_all, 2, '.', ''); ?></span> 
           &nbsp; | &nbsp;
           <span style="font-weight: bold;">Total Subsidy (Company ) : <?php echo number_format((float)$total_cash_subsidy_all, 2, '.', ''); ?></span> 
           
           &nbsp; | &nbsp;  
           <span style="font-weight: bold;">Total : <?php $total=number_format((float)$total_cash_all, 2, '.', '')+number_format((float)$total_cash_tagged_all, 2, '.', '')+number_format((float)$total_subsidy_all, 2, '.', '')+number_format((float)$total_bcml_tagged_all, 2, '.', '')+
...
<span style="float: right;font-weight: bold;color: #933B3B;">Note : C-Tagged=>Cash Tagged, C-Subsidy=>Cash Taken by store incharge for subsidy order</span>
	...
	<td class="text-right">Store Cash</td>
              ...
              <td class="text-right">Store C-Subsidy</td>
                ...
                <td class="text-right" style="display:none;">Store Tagged</td> 
	...
	<td class="text-right">No. Order (Tagged Subsidy)</td>
	...
	<td class="text-right" style="display:none;"><?php echo $order['tagged']; ?></td>
	...
	<td class="text-right"><?php echo $order['Tagged_subsidy_order']; ?></td>
	...
	<span style="font-weight: bold;">Page Total  :: </span> 
	 ...
	 <span style="font-weight: bold;">Total C-Subsidy : <?php echo number_format((float)$total_Cash_subsidy, 2, '.', '');; ?></span>
		   ...
		   &nbsp; | &nbsp;
           <span style="font-weight: bold;">Total BCML Tagged : <?php echo number_format((float)$total_bcml_tagged, 2, '.', '');; ?></span> 
           &nbsp; | &nbsp;
           <span style="font-weight: bold;">Total Subsidy (By Company): <?php echo number_format((float)$total_subsidy, 2, '.', '');; ?></span> 
           
           &nbsp; | &nbsp; 
           <span style="font-weight: bold;">Total : <?php $total=number_format((float)$total_cash, 2, '.', '')+number_format((float)$total_bcml_tagged, 2, '.', '')+number_format((float)$total_subsidy, 2, '.', '')+number_format((float)$total_Cash_Tagged, 2, '.', '')+$total_Cash_subsidy; echo $total; ?></span> 
           <br/>
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=reportbcml/sale_summary/sale_summary&token=<?php echo $token; ?>';
...
$('#button-download').on('click', function() {
    url = 'index.php?route=reportbcml/sale_summary/download_excel_sale_summary&token=<?php echo $token; ?>';
    	var filter_date_start = $('input[name=\'filter_date_start\']').val();
