...
<i class="<?php echo $tool_tip_class; ?> " data-toggle="tooltip" style="<?php echo $tool_tip_style; ?>" title="<?php echo $tool_tip; ?>"></i>
  ...
  <div class="container-fluid">
      ...
      <div class="panel-body">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <div  class="input-group date" id="date_from">
                  <input readonly="readonly" disabled="disabled" type="text" name="filter_date_start" value="<?php echo $filter_date_start; ?>" placeholder="<?php echo $entry_date_start; ?>" data-date-format="YYYY-MM-DD" id="input-date-start" class="form-control" />
              ...
              <div class="form-group">
                ...
                <div class="input-group date" id="date_to">
                  <input readonly="readonly" disabled="disabled"  type="text" name="filter_date_end" value="<?php echo $filter_date_end; ?>" placeholder="<?php echo $entry_date_end; ?>" data-date-format="YYYY-MM-DD" id="input-date-end" class="form-control" />
                  <span class="input-group-btn">
                  <button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button>
                  </span></div>
              </div>
              <div class="form-group">
                <label class="control-label" for="input-date-end">Billing Type</label>
                
                  
                  <select   name="filter_billing_type" id="input-filter_billing_type" class="form-control">
            ...
            <div class="col-sm-6">
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <select readonly="readonly" disabled="disabled"  name="filter_store" id="input-store" class="form-control">
		...
		<div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <select readonly="readonly" disabled="disabled"  name="filter_unit" id="input-unit" class="form-control">
	...
	<span style="font-weight: bold;">Total Amount : <?php echo number_format((float)$total_tagged_amount_All, 2, '.', ''); ?></span> 
                  <button type="button" id="button-bill" class="btn btn-primary pull-right" 
                          onclick="check_count()" style="margin-right: 10px !important;">
            Print Bill</button>         
                           <br/><br/>
            ...
            <thead>
              <tr>
                  <td class="text-left" style="width: 20px;">
                    <input <?php if($_SESSION['all_selected']=='true'){ echo 'checked="checked"'; } ?>
              ...
              <tr>
                  <td class="text-left"><input 
                      <?php if($order['selected']=='true'){ echo 'checked="checked"'; } ?>
                          ...
                          type="checkbox" id="check_box_<?php echo $order['inv_no']; ?>" onclick="return add_to_array('<?php echo $order['inv_no']; ?>','check_box_<?php echo $order['inv_no']; ?>');" name="checkd_bill"  /></td>
  ...
  <div class="modal fade" id="myModal_create_bill" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <form action="index.php?route=report/reconciliation/create_bill_1&token=<?php echo $token; ?>" method="post" enctype="multipart/form-data" onsubmit="return myFunction()" >
            <div class="form-group">
...
{
    url = 'index.php?route=report/reconciliation/check_selected_orders_count&token=<?php echo $token; ?>';
    ...
    $.ajax({
              ...
              // dataType: 'json',
               ...
               {
                           //alert(json);	
                          // if(json=='all_selected')
                           {
                                //alertify.success('Good now You can take print');
                                url = 'index.php?route=report/reconciliation/create_bill&token=<?php echo $token; ?>';
		    var filter_date_start = $('input[name=\'filter_date_start\']').val();
	
		if (filter_date_start) 
		{
			url += '&filter_date=' + encodeURIComponent(filter_date_start);
		}
		...
		if (filter_store != 0) 
		{
			url += '&filter_store_2=' + encodeURIComponent(filter_store);
		}
		var filter_unit = $('select[name=\'filter_unit\']').val();
	
		if (filter_unit != '') 
		{
			url += '&filter_unit_2=' + encodeURIComponent(filter_unit);
		}
		var filter_billing_type = $('select[name=\'filter_billing_type\']').val();	
		if (filter_billing_type) 
		{
			url += '&filter_billing_type=' + encodeURIComponent(filter_billing_type);
		}
			location = url;
                           }
                           //else
...
{       //alert(order_id);
        ...
        {
            //alert('checked');
            url = 'index.php?route=report/reconciliation/add_orders_to_array&token=<?php echo $token; ?>&order_id='+order_id+'&action=add';
        ...
        {
            //alert('unchecked');
            url = 'index.php?route=report/reconciliation/add_orders_to_array&token=<?php echo $token; ?>&order_id='+order_id+'&action=remove';
            $.ajax({
              url: url,
              // dataType: 'json',
               success: function(json) 
 ...
 function add_all_to_array()
 {
     if($('#check_box_all').is(':checked'))
        {
            //alert('checked');
            url = 'index.php?route=report/reconciliation/select_all_bills&token=<?php echo $token; ?>&action=add';
            var filter_date_start = $('input[name=\'filter_date_start\']').val();
            ...
            $.ajax({
              ...
              // dataType: 'json',
               success: function(json) 
               {
                        ...
                        alertify.success('All Bills are Selected');
               }
        ...
        {
            //alert('unchecked');
            url = 'index.php?route=report/reconciliation/select_all_bills&token=<?php echo $token; ?>&action=remove';
            $.ajax({
              url: url,
              // dataType: 'json',
               success: function(json) 
               {
                           ...
                           $('#checkboxes').find('input:checkbox').prop('checked', false); 
                            alertify.error('All Bills are Un Selected');
               }
