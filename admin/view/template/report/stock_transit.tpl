...
<i class="<?php echo $tool_tip_class; ?> " data-toggle="tooltip" style="<?php echo $tool_tip_style; ?>" title="<?php echo $tool_tip; ?>"></i>
  ...
  <div class="container-fluid">
	...
	<?php if($user_group_id==1){ ?>
	<td class="text-right">
<img id="search_img_<?php echo $order['order_id']; ?>" src="view/image/processing_image.gif" style="height: 48px;width: 48px;display: none;" />
                <a id="cncl_btn_<?php echo $order['order_id']; ?>" href="#" onclick="return cancel_order(<?php echo $order['order_id']; ?>);" class="btn btn-primary">Cancel Transfer   <a/>
           </td>
         <?php  } ?>         
              </tr>              <?php } ?>
              <?php } else { ?>
  ...
  <script type="text/javascript">
	function cancel_order(order_id)
    {
	  ...
	  if(cnfrm)
	  {
		var url='index.php?route=report/stock/cancel_transit_order&token=<?php echo $token; ?>&order_id=' + encodeURIComponent(order_id);
		//alert(url);
		$.ajax({
		url: url,
		dataType: 'json',
		beforeSend: function() 
                {  
                    $("#search_img_"+order_id).show();
                    $("#cncl_btn_"+order_id).hide();
	      
                },
		complete: function() 
                {
                    $("#search_img_"+order_id).hide();
                    $("#cncl_btn_"+order_id).hide();
		...
		success: function(html) 
                {
                    ...
                    if(html>0)
                    {
                        alertify.success('Order Cancel Successfully');
						$("#search_img_"+order_id).hide();
						$("#cncl_btn_"+order_id).hide();
						location.reload();
                       return false;
                    ...
                    {
						alertify.error('You are not authorized for this transaction');
                        $("#search_img_"+order_id).hide();
						$("#cncl_btn_"+order_id).show();
                       return false;
                    }
                    else
                    {
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=report/stock/transit&token=<?php echo $token; ?>';
...
$('#button-download').on('click', function() {
	url = 'index.php?route=report/stock/download_transit&token=<?php echo $token; ?>';
