...
?>
        ...
        <div class="col-md-12" >                                   
			...
			Select All at this Page : <input title="Click here to Select/Un-Select All Bills on this page" type="checkbox" id="check_box_all_page"
			onclick="return add_all_to_array_page();" name="checkd_bill_all_page" /> 
...
type="checkbox" class="check_boxes" id="check_box_<?php echo $order['GROWER_ID']; ?>" onclick="return add_to_array('<?php echo $order['GROWER_ID']; ?>','check_box_<?php echo $order['GROWER_ID']; ?>');" name="checkd_bill" /></td>
...
function add_all_to_array_page()
{
	if($('#check_box_all_page').is(':checked'))
	{
		$('input.check_boxes:checkbox').each(function () 
		{
		var sThisVal = $(this).val();
		var iddd=$(this).attr('id');
		//alert(iddd);
		idd2=iddd.split("check_box_");
		
		//alert(idd2[1]);
		url = 'index.php?route=farmerrequest/cardprint/add_orders_to_array&token=<?php echo $token; ?>&order_id='+idd2[1]+'&action=add';
		//alert(url);
		$.ajax({
		url: url,
		// dataType: 'json',
		...
		alertify.success('All are selected at this page');
	}
