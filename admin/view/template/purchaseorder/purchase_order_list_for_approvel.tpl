...
<div id="content">
  ...
  <div class="container-fluid">
	...
	<div class="col-sm-6">
			...
			<div class="form-group">
            <label class="control-label" for="input-date-end">PO Number (ASPL/PO/)</label>
            <input name="filter_po" id="input-filter_po" value="<?php echo $filter_po; ?>" required="required" style="width: 100%;" class="form-control" placeholder="PO NUmber" />
		...
		<option <?php if ($filter_status == '3') { ?> selected="selected" <?php } ?> value="3" >PR Linked</option>
            </select>
            </div>
        </div>    
		...
		<td class="text-left">PO Date</td>
                    ...
                    <td class="text-left">Status</td>
						...
						<td class="text-left">Approved Status</td>
						<td class="text-left">Approved By</td>
						<td class="text-left">Approved Date</td>
              ...
              <tbody>
                ...
                if($order_list)
                {
                    ...
                    { 
                        //echo "<pre>"; print_r($order['status']);echo "</pre>"; exit;
		?>
                    <tr>
		...
		<td class="text-left"><?php echo $order['create_date']; ?></td>	
	          <td class="text-left"><?php echo $order['id_prefix'].$order['sid']; ?></td>
		       ...
		       if($order['status']=='0') 
						...
						<td class="text-left"><?php echo $order['create_by']; ?></td>
						<td class="text-left"><?php  
		       if($order['approve_status']=='0'  ) 
                                   {
                                     echo "Pending";
                                   }
                                   else if($order['approve_status']=='1' || $order['status'] =='3' ) 
                                   {
                                    echo "Approved , PO Image Link" ;
                                   }
								   else if($order['approve_status']=='2' ) 
                                   {
                                    echo "Rejected";
                                   }
                                   else 
                            ...
                            ?></td>
                        <td class="text-left"><?php echo $order['approved_date']; ?></td>
                        <td class="text-left"><?php echo $order['approved_by']; ?></td>
		...
		<?php  
		       if($order['approve_status']=='0') 
               {
                            ...
                            ?>
							<br/><br/>
		<a href="<?php echo $approve.'&sid='.$order['sid'].'&supplier_id='.$order['supplier_id'].'&email_send='.$order['email_send']; ?>"  onclick='return confirm("Are You Sure ? Want to Approve PO!")' data-toggle="tooltip" title="Approve PO" style="margin-left: 5px;background-color: rgb(104, 185, 104);" class="btn btn-info">
                            <i class="fa fa-check" ></i> 
                            </a>
							<br/><br/>
							<a href="<?php echo $reject.'&sid='.$order['sid'].'&supplier_id='.$order['supplier_id'].'&email_send='.$order['email_send']; ?>"  onclick='return confirm("Are You Sure ? Want to Reject PO!")' data-toggle="tooltip" title="Reject PO" style="margin-left: 5px;background-color: red;" class="btn btn-info">
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=purchaseorder/purchase_order/approve_po&token=<?php echo $token; ?>';
	...
	if (filter_supplier) {
	   ...
	   var filter_po = $('#input-filter_po').val();
        if (filter_po) {
