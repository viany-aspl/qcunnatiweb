...
<?php if ($error) {  ?>
	...
	<span style="font-weight: bold;">Total Amount : <?php echo number_format((float)$total_subsidy_amount_all, 2, '.', ''); ?></span> 
	...
	<td class="text-right"><?php echo $order['o_bill_no']; ?></td>
				...
				<td class="text-right" id="td_<?php echo $order['inv_no']; ?>">
				<?php if(($order['dscl_upload']==0) || ($order['dscl_upload']==2))
				{ 
				?>
				<img id="sbmt_img_<?php echo $order['inv_no']; ?>" src="view/image/processing_image.gif" style="height: 48px;width: 48px;display: none;" />
				<a href="#" id="submit_btn_<?php echo $order['inv_no']; ?>" class="btn btn-primary pull-right" onclick="return submit_order_to_dscl(<?php echo $order['inv_no']; ?>,<?php echo $order['dscl_upload']; ?>);">
					Upload 
				</a>
				<?php }
					else
		...
		<span style="font-weight: bold;">Page Total :: Total Amount : <?php echo number_format((float)$total_subsidy_amount, 2, '.', ''); ?></span> 
  ...
  <div class="modal fade" id="myModal_create_bill" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <form action="index.php?route=reportdscl/reconciliationsubsidy/create_bill_subsidy&token=<?php echo $token; ?>" method="post" enctype="multipart/form-data" onsubmit="return myFunction()" >
            <div class="form-group">
...
$("#input-store").select2();
function submit_order_to_dscl(order_id,dscl_upload)
{
	//alert(order_id);
	url = 'index.php?route=reportdscl/reconciliationsubsidy/submit_order_to_dscl&token=<?php echo $token; ?>&order_id='+order_id+'&dscl_upload='+dscl_upload;
	$.ajax({
              url: url,
              // dataType: 'json',
			  beforeSend: function() 
			  {
					$("#submit_btn_"+order_id).hide();
					$("#sbmt_img_"+order_id).show();
               ...
               success: function(json) 
               {
					...
					if(json=='1')
					{
						alertify.success('Order Delivery submitted successfully');
						$("#submit_btn_"+order_id).hide();
						$("#sbmt_img_"+order_id).hide();
						$("#td_"+order_id).html('Submitted');
					}
					else if(json=='2')
					{
						alertify.error('Multiple product found in order.');
						$("#submit_btn_"+order_id).show();
						$("#sbmt_img_"+order_id).hide();
					}
					else
					{
						alertify.error('oops! some error occur in updation.Please try again');
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=reportdscl/reconciliationsubsidy/subsidy&token=<?php echo $token; ?>';
...
$('#button-download').on('click', function() {
	url = 'index.php?route=reportdscl/reconciliationsubsidy/download_excel_subsidy&token=<?php echo $token; ?>';
...
$('#button-pdf').on('click', function() {
	url = 'index.php?route=reportdscl/reconciliationsubsidy/download_pdf_subsidy&token=<?php echo $token; ?>';
...
$('#button-details').on('click', function() {
	url = 'index.php?route=reportdscl/reconciliationsubsidy/download_item_excel_subsidy&token=<?php echo $token; ?>';
