...
<select name="filter_fm" id="input-fm" class="form-control select2"  style="display: none;width: 100%;">
...
<br/>
	...
	<table class="table table-bordered" id="tbl_total_main" style="display: none;">
            <thead>
               <tr>
				...
				<td class="text-left" id="subsidy_clm" style="display: none;">SUBSIDY AMOUNT :  </td>
				<td class="text-left" id="cash_clm" style="display: none;">CASH AMOUNT :  </td>
  ...
  <input type="hidden" id="allow_subsidy_total" value="" />
  <input type="hidden" id="allow_cash_total" value="" />
  ...
  <div id="" class="for_print" style="display: none;font-size: 90%;" >
  <div class="scrollbar_wrapper" id="scrollbar2" style="">  
		...
		<br />
					  ...
					  <div class="order_id pull-left">
					   ...
					   <br />
					   <div class="order_id pull-left" id="prnt_cat" style="display:none;">
						
                           Subsidy Category -  <span id="prnt_cat_span"></span>
                      </div>
					  </br>
					  <div class="order_id pull-left">
                         Payment Method -  <span id="prnt_pm"></span>
                      </div>
					  <br /><br />
                    </div>  
                      <hr/>
                  ...
                  <div style="text-align: center;">
					...
					<br /><br />
					<div class="stor_logo pull-left">
                  <span id="print_pay_by_company" style="display: none"></span><br />
					<span id="print_pay_by_customer" style="display: none"></span><br />				  
					* Tagged Amount : <span id="print_tagged_amount"></span><br />
					<span id="print_subsidy_amount" style="display: none"></span><br />
					* Cash Amount : 0
					<br /><br />
                  </div>
					  <br /><br /><br /><br />
...
$("#input-fm").select2().next().hide();
//$("#input-fm").select2().next().show();
	//print();
	function print() {   
		//alert('kk');
		//scrollbar_wrapper
	...
	function submit_order()
	{
		...
		var subsidy_total=$("#allow_subsidy_total").val();
		var cash_total=$("#allow_cash_total").val();
		
		var allow_tagged_total=parseFloat($("#allow_tagged_total").val())+parseFloat(subsidy_total);
	  ...
	  $.ajax({
		...
		success: function(html) {
			...
			for(var icount=0;icount<(html['detail']['products']).length;icount++ )
			{
				...
				obj.ActAmount=html['detail']['products'][icount]['Acttotal'];
				obj.ActRate=html['detail']['products'][icount]['ActRate'];
				obj.SubSidyPer=html['detail']['products'][icount]['SubSidyPer'];
				obj.SubsidyAmount=html['detail']['products'][icount]['Subsidytotal'];
				obj.bcml_code=html['detail']['products'][icount]['BCMLCODE'];
				obj.S_CODE=html['detail']['products'][icount]['S_CODE'];
				obj.S_DESC=html['detail']['products'][icount]['S_DESC'];
				obj.SubRate=html['detail']['products'][icount]['SubRate'];
				prd_array.push(obj); 
				S_DESC=S_DESC+html['detail']['products'][icount]['S_DESC']+'/';
			...
			S_DESC = S_DESC.substring(0,(S_DESC.length-1));
			...
			$("#allow_subsidy_total").val(html['detail']['subsidytotal']);
			$("#allow_cash_total").val(html['detail']['cashtotal']);
			console.log(html['detail']);
			console.log(prd_array);
			if(html['detail']['subsidytotal']!=0)
			{
				$("#subsidy_clm").html('SUBSIDY AMOUNT : '+html['detail']['subsidytotal']);
				$("#subsidy_clm").show();
			}
			else
			{
			...
			$("#cash_clm").html('CASH AMOUNT : '+html['detail']['cashtotal']);
			$("#cash_clm").show();
			...
			//////////////add data in print div///////////////
				...
				$("#print_pay_by_customer").html('* Pay By Customer : '+html['detail']['total']);
				$("#print_pay_by_customer").show();
				console.log('subsidytotal'+html['detail']['subsidytotal']);
				if(html['detail']['subsidytotal']!=0)
				{
					
					$("#prnt_cat_span").html(S_DESC);
					$("#print_subsidy_amount").html('* Subsidy Amount : '+html['detail']['subsidytotal']);
					$("#print_pay_by_company").html('* Pay By Company : '+html['detail']['subsidytotal']);
					$("#print_subsidy_amount").show();
					$("#print_pay_by_company").show();
					$("#prnt_cat").show();
					$("#prnt_pm").html('Tagged Subsidy');
				}
				else
				{
					$("#print_subsidy_amount").hide();
					$("#print_pay_by_company").hide();
					$("#prnt_cat").hide();
					$("#prnt_pm").html('Tagged');
				}
			//////////////////////////
