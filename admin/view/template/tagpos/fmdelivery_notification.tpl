...
<div id="content">
  <div class="page-header">
    <div class="container-fluid">
      <h1><?php echo "FM Delivery Notification"; ?></h1>
      <ul class="breadcrumb">
...
<?php if ($error) {  ?>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-bar-chart"></i> <?php echo "FM Delivery Notification"; ?></h3>
			 ...
			 <div class="form-group">
			  <label class="control-label" for="input-invoice">Invoice</label>
                <div class="input-group col-sm-12">
                  <input type="text" name="filter_invoice" value="<?php echo $filter_invoice!=0?$filter_invoice:''; ?>" placeholder="Invoice No." id="input-invoice" class="form-control" />
                </div>
			 </div>
		...
		<td class="text-right">Order Date</td>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-right">
                    <a href="javascript:void(0);"<?php echo $order['status']=='1'?' disabled="disabled"':''; ?> data-toggle="modal" data-target="#myModal_invoice" data-store="<?php echo $filter_store;?>" data-id="<?php echo $order['inv_no'];?>" data-stats="<?php echo $order['status'];?>" data-title="FM Delivery Invoice( <?php echo $order['inv_no'];?> )" class="btn btn-info btn-small"><i class="fa fa-s" title="Delivered"></i>Set Delivered</a>                                        
                </td>
              </tr>              <?php $aa++; } ?>
              <?php  } else { ?>
              <tr>
...
<!-- Modal -->
  <div class="modal fade" id="myModal_invoice" role="dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">FM Delivery Invoice</h4>
        </div>
        <div class="modal-body">
        </div>
        <div class="modal-footer">
            <button type="button" name="send" class="btn btn-info btn-SendSMS">Send SMS Notification</button>
  ...
  <script type="text/javascript">
      jQuery('#myModal_invoice').on('show.bs.modal',function(e){
          ...
          var url = 'index.php?route=tagpos/fmdelivery/invoiceDetail&token=<?php echo $token; ?>';
          jQuery(e.currentTarget).find('.modal-title').text(jQuery(e.relatedTarget).data('title'));
          jQuery.post(url,{filter_store:store,filter_invoice:inv_id},function(dta){
              if(dta.status=='error'){
      ...
      jQuery('#myModal_invoice').on('click','.btn-SendSMS',function(){
          var invoice_id = jQuery(this).data('id');
          var mobile = jQuery(this).data('mobile');
//          alert(invoice_id+' '+mobile);
//          return false;
          jQuery.post('index.php?route=tagpos/fmdelivery/SendInvoiceSMSNotification&token=<?php echo $token; ?>',
          {invoice_id:invoice_id,customer_mob:mobile},function(data){
              if(data.status == 'success'){
                  jQuery('.btn-SendSMS').attr('disabled','disabled');
...
//	  return false;
	...
	url = 'index.php?route=tagpos/fmdelivery/deliveryNotification&token=<?php echo $token; ?>';
